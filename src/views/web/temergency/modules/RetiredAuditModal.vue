    ...
    <a-spin :spinning="confirmLoading">
      <div class="dialogBody">
        <a-tabs default-active-key="1" @change="callback">
            ...
            <a-card :bordered="false">
              ...
              <a-row class="mbot" v-if="fileList.length" style="margin-top: 15px">
                <a-col :span="2" :offset="1">
                  <a-button style="background: #40a9ff; color: #fff" @click="downAll(model.id)">一键打包下载</a-button>
                </a-col>
              </a-row>
              <!-- <span v-if="fileList.length == 0">暂无附件</span>
              <ul
                class="filelist"
            ...
            <a-tabs defaultActiveKey="0" v-if="historyList.length">
              <a-tab-pane v-for="(item, index) in historyList" :key="index" :tab="'历史事件' + (index + 1)">
                ...
                <a-card :bordered="false">
                  ...
                  <a-row class="mbot" v-if="attachList[index]" style="margin-top: 15px">
                    <a-col :span="2" :offset="1">
                      <a-button style="background: #40a9ff; color: #fff" @click="downAll(item.id)">一键打包下载</a-button>
                    </a-col>
                  </a-row>
                </a-card>
              </a-tab-pane>
          ...
          <a-tab-pane key="6" tab="退工办理记录" v-if="showFee">
        ...
        <a-tabs defaultActiveKey="0">
          ...
          >
            ...
            <a-card :bordered="false">
              ...
              <detail-list>
                ...
                <detail-list-item term="商业保险">
                  <a-tooltip placement="topLeft">
                    ...
                    <span class="overflow">{{ item.insuranceStr || '未购买' }}</span>
                  </a-tooltip>
                </detail-list-item>
              </detail-list>
              <detail-list>
              ...
              <detail-list>
                <detail-list-item term="皖信工龄">{{ item.workLengthWxYear || '0' }}年{{ item.workLengthWxMonth || '0' }}月</detail-list-item>
                <detail-list-item term="前12个月平均工资">{{ item.averageSalary || '--' }}</detail-list-item>
              </detail-list>
              <detail-list>
                <detail-list-item term="客户单位工龄">{{ item.workLengthCustomerYear || '0' }}年{{ item.workLengthCustomerMonth || '0' }}月</detail-list-item>
            ...
            <a-card :bordered="false">
              ...
              <detail-list>
                <detail-list-item term="事件类别">{{ item.eventTypeName }}</detail-list-item>
              </detail-list>
  ...
  data() {
    return {
      ...
      url: {
        ...
        getDetailById: '/hrEmergency/tretiredworkevent/', //获取当条记录详情
        ...
        extraUrl: '/hrEmergency/tretiredworkevent/toSupplemented', //材料待补充
        backUrl: '/hrEmergency/tretiredworkevent/back', //退回地址
        accessUrl: '/hrEmergency/tretiredworkevent/auditPass', //审核通过
        getAttachUrl: '/hrEmergency/fileUpload/ossFileUrl/domain/', //获取附件列表
        toMajor: '/hrEmergency/tretiredworkevent/toMajor/', //转为重大
  ...
  methods: {
    ...
    callback(key) {
      ...
      if (key == 3 && this.processList.length == 0) {
        httpAction(this.url.getRecordUrl + '?id=' + this.id + '&type=2', null, 'get').then((res) => {
          if (res.code == 200 && res.data) {
      ...
      if(key == 6){
        //获取记录
        httpAction(this.url.injuryClaims+'?id='+this.model.id+'&payMainType=0&type=2',null,'get').then((res)=>{
          if(res.code == 200){
    ...
    edit(record,show,statusTxt) {
      ...
      this.id = record.retiredWorkEvent.id
      this.visible = true
      ...
      this.fileList = []
      //获取附件信息
      httpAction(this.url.getAttachUrl + this.model.id + '?type=2', null, 'get').then((res) => {
        if (Number(res.code) === 200 && res.data) {
    ...
    exportExcel() {
      const tabData = [
        ...
        {
          label1: '社会保险',
          text1: this.socialStatus[this.model.socialStatus],
          label2: '',
          text2: '',
          label3: '',
          text3: '',
        ...
        { label1: '事件说明', text1: this.model.eventThrough, label2: '', text2: '', label3: '', text3: '' },
      ...
      this.json2excel(excelDatas, `退工事件-${this.model.emName}`, true, 'xlsx', 2, this.fileList.length)
    },
    ...
    history() {
      ...
      const _this = this
      httpAction('/hrEmergency/tretiredworkevent/old/' + this.model.empId, null, 'get').then((res) => {
        if (res.code == 200 && res.data) {
          for (let [index, item] of res.data.entries()) {
            _this.$set(_this.historyList, index, item.retiredWorkEvent)
            //获取附件
            httpAction(_this.url.getAttachUrl + item.retiredWorkEvent.id + '?type=' + _this.flag, null, 'get').then(
              (result) => {
            ...
            _this.$set(
              ...
              `departName`,
              this.settleDomainOptions[item.retiredWorkEvent.emSettleId]
                ? this.settleDomainOptions[item.retiredWorkEvent.emSettleId].departName
                : ''
            )
            _this.$set(
              _this.historyList[index],
              `eventTypeName`,
              filterDictText(this.eventTypeOptions, item.retiredWorkEvent.eventType)
            )
            ...
            httpAction(
              this.url.getCustomerById +
                '?cusId=' +
                item.retiredWorkEvent.emCusId +
                '&settleDomainId=' +
                item.retiredWorkEvent.emSettleId,
              null,
              'get'
            ).then((ress) => {
      ...
      downFile(httpurl, null).then((res) => {
        ...
        fileReader.onload = function () {
          try {
            ...
            downloadElement.download = _this.model.emName + '的材料附件' + '.zip' // 下载后文件名
