    ...
    <a-spin :spinning="confirmLoading">
      <div class="dialogBody">
        <a-tabs default-active-key="1" @change="callback">
          <a-tab-pane key="1" tab="详情">
            ...
            <table class="detailTable">
              ...
              <tr v-if="model.eventType == 5">
                <td colspan="2">
                  <label>车辆归属单位</label>
                  <span>{{ model.carOfUnit }}</span>
                </td>
                <td>
                  <label>交强险赔付额</label>
                  <span>{{ model.payStrongInsurance }}</span>
                </td>
              </tr>
              <tr v-if="model.eventType == 5">
                <td colspan="2">
                  <label>车辆保险购买单位</label>
                  <span>{{ model.carInsuranceUnit }}</span>
                </td>
                <td>
                  <label>第三责任险保额</label>
                  <span>{{ model.thirdPartyLiabilityInsurance }}</span>
                </td>
              </tr>
              <tr v-if="model.eventType == 5">
                <td>
                  <label>伤者户口归属</label>
                  <span>{{ model.woundedHukouAttribution == 0 ? '城镇居民' : '农村户口' }}</span>
                </td>
                <td>
                  <label>受伤部位</label>
                  <span>{{ model.injuredArea }}</span>
                </td>
                <td>
                  <label>受伤程度</label>
                  <span>{{ model.injuredDegree }}</span>
                </td>
              </tr>
              <tr v-if="model.eventType == 5">
                <td>
                ...
                <td colspan="2">
                  <label>第三人身份号</label>
                  <span>{{ model.thirdPersonIdCard }}</span>
                </td>
              </tr>
              <tr v-if="model.eventType == 5">
                <td>
                  <label>伤者家庭情况</label>
                  <span>{{ model.injuredFamilyStatus }}</span>
                </td>
                <td colspan="2">
                  <label>治疗费（预判）</label>
                  <span>{{ model.treatmentCosts }}</span>
                </td>
              </tr>
              <tr>
                <td colspan="3">
            ...
            <table class="detailTable">
              ...
              <tr v-if="model.superviseOpinion">
                <td colspan="3">
            ...
            <a-card :bordered="false">
              ...
              <a-row class="mbot" v-if="fileList.length">
                <a-col :span="2" :offset="1" style="margin-top: 10px">
                  <a-button type="primary" @click="downAll(model.id)">一键打包下载</a-button>
                </a-col>
              </a-row>
          ...
          <a-tab-pane key="4">
            ...
            <a-empty v-if="historyList.length == 0" description="暂无历史附件" />
              ...
              <a-tab-pane v-for="(item, index) in historyList" :key="index" :tab="'历史事件' + (index + 1)">
                ...
                <table class="detailTable">
                  ...
                  <tr v-if="item.eventType == 5">
                    <td colspan="2">
                      <label>车辆归属单位</label>
                      <span>{{ item.carOfUnit }}</span>
                    </td>
                    <td>
                      <label>交强险赔付额</label>
                      <span>{{ item.payStrongInsurance }}</span>
                    </td>
                  </tr>
                  <tr v-if="item.eventType == 5">
                    <td colspan="2">
                      <label>车辆保险购买单位</label>
                      <span>{{ item.carInsuranceUnit }}</span>
                    </td>
                    <td>
                      <label>第三责任险保额</label>
                      <span>{{ item.thirdPartyLiabilityInsurance }}</span>
                    </td>
                  </tr>
                  <tr v-if="item.eventType == 5">
                    <td>
                      <label>伤者户口归属</label>
                      <span>{{ item.woundedHukouAttribution == 0 ? '城镇居民' : '农村户口' }}</span>
                    </td>
                    <td>
                      <label>受伤部位</label>
                      <span>{{ item.injuredArea }}</span>
                    </td>
                    <td>
                      <label>受伤程度</label>
                      <span>{{ item.injuredDegree }}</span>
                    </td>
                  </tr>
                  <tr v-if="item.eventType == 5">
                    <td>
                    ...
                    <td colspan="2">
                      <label>第三人身份号</label>
                      <span>{{ item.thirdPersonIdCard }}</span>
                    </td>
                  </tr>
                  <tr v-if="item.eventType == 5">
                    <td>
                      <label>伤者家庭情况</label>
                      <span>{{ item.injuredFamilyStatus }}</span>
                    </td>
                    <td colspan="2">
                      <label>治疗费（预判）</label>
                      <span>{{ item.treatmentCosts }}</span>
                    </td>
                  </tr>
                  <tr>
                    <td colspan="3">
          ...
          <a-tab-pane key="5" tab="工伤理赔" v-if="showFee">
            <handle-record ref="HandleRecordForm1" :feeTypeOption="feeTypeOption" @ok="handleCancel"></handle-record>
          </a-tab-pane>
          <a-tab-pane key="6" tab="商险理赔" v-if="showFee">
            <handle-record ref="HandleRecordForm2" :feeTypeOption="feeTypeOption" @ok="handleCancel"></handle-record>
          </a-tab-pane>
        </a-tabs>
      </div>
      ...
      <a-row class="dialogBottom" v-if="isOperate">
        <a-col :span="11" :offset="12">
          <a-button
            class="changeEventBtn"
            @click="operateDialog('event', true)"
            v-has="'wxhr:tworkinjuryevent_to_major'"
          >转为重大事件</a-button>
          <a-button class="accessBtn" v-has="'wxhr:tworkinjuryevent_audit_pass'" @click="operateDialog('access', true)">审核通过</a-button>
          <a-button
            class="extraBtn"
            v-has="'wxhr:tworkinjuryevent_to_supplemented'"
            @click="operateDialog('extra', true)"
          >待补充</a-button>
          <a-button class="backBtn" v-has="'wxhr:tworkinjuryevent_back'" @click="operateDialog('back', true)">退回</a-button>
...
export default {
  ...
  data() {
    return {
      ...
      validatorRules: {
        info: {
          rules: [
            { required: true, message: '请填写待补充内容!' },
      ...
      url: {
        ...
        extraUrl: '/hrEmergency/tworkinjuryevent/toSupplemented', //材料待补充
        backUrl: '/hrEmergency/tworkinjuryevent/back', //退回地址
        accessUrl: '/hrEmergency/tworkinjuryevent/auditPass', //审核通过
        getAttachUrl: '/hrEmergency/fileUpload/ossFileUrl/domain/', //获取附件列表
        toMajor: '/hrEmergency/tworkinjuryevent/toMajor/', //转为重大
  ...
  methods: {
    ...
    edit(record,show) {
      ...
      this.visible = true
      this.model = Object.assign({ insurance: record.insurance }, record.workInjuryEvent)
    ...
    callback(key) {
      ...
      if (key == 3 && this.processList.length == 0) {
        httpAction(this.url.getRecordUrl + '?id=' + this.id + '&type=0', null, 'get').then((res) => {
          if (res.code == 200 && res.data) {
      ...
      if (key == 5 && this.requestFlag.gsflag){
       this.requestFlag.gsflag = false;
        //获取记录
        httpAction(this.url.socialClaims+'?id='+this.model.id+'&payMainType=0&type=0',null,'get').then((res)=>{
          if(res.code == 200){
            this.$refs.HandleRecordForm1.status = filterDictText(this.auditStatusOption, this.model.auditStatus);
            this.$refs.HandleRecordForm1.show(this.model,res.data,0);
          }else{
            this.$message.warning(res.msg);
          }
        })
      ...
      if (key == 6 && this.requestFlag.sxflag){
        this.requestFlag.sxflag = false;
        //获取记录
        httpAction(this.url.socialClaims+'?id='+this.model.id+'&payMainType=1&type=0',null,'get').then((res)=>{
          if(res.code == 200){
            this.$refs.HandleRecordForm2.status = filterDictText(this.auditStatusOption, this.model.auditStatus)
            this.$refs.HandleRecordForm2.show(this.model,res.data,1)
          }else{
            this.$message.warning(res.msg)
          }
        })
    ...
    history() {
      ...
      const _this = this
      httpAction('/hrEmergency/tworkinjuryevent/old/' + this.model.empId, null, 'get').then((res) => {
        if (res.code == 200 && res.data) {
          for (let [index, item] of res.data.entries()) {
            _this.$set(_this.historyList, index, item.workInjuryEvent)

            //获取附件
            httpAction(_this.url.getAttachUrl + item.workInjuryEvent.id + '?type=' + _this.flag, null, 'get').then(
              (result) => {
            ...
            _this.historyList[index].curingTypeName = filterDictText(
              ...
              item.workInjuryEvent.curingType
            ) //治疗类型
            // 结算主体
            _this.historyList[index].departName = this.settleDomainOptions[item.workInjuryEvent.emSettleId]
              ? this.settleDomainOptions[item.workInjuryEvent.emSettleId].departName
              : ''
            //商险
            let insuranceStr = ''
            if (item.insurance) {
            ...
            const riskBearName =
              item.workInjuryEvent.riskBear == 1
                ? getNameFromTree(this.customerTreeData, item.workInjuryEvent.riskBearUnit, 'value', 'title')
                : getNameFromArr(this.organizationOption, item.workInjuryEvent.riskBearOrgan, 'id', 'organName')
            _this.$set(_this.historyList[index], `riskBearName`, riskBearName)
            //查询客户单位合同相关信息
            httpAction(
              this.url.getCustomerById +
                '?cusId=' +
                item.workInjuryEvent.emCusId +
                '&settleDomainId=' +
                item.workInjuryEvent.emSettleId,
              null,
              'get'
            ).then((ress) => {
    ...
    preview(file) {
      ...
      if (/xls|xlsx|doc|docx|ppt|pptx|pdf/gi.test(fileExt)) {
        window.open(
    ...
    downAll(id) {
      let httpurl = this.url.down + id + '?type=0'
      const _this = this
      downFile(httpurl, null).then((res) => {
        ...
        fileReader.onload = function () {
          try {
            ...
            downloadElement.download = _this.model.emName + '的工伤材料附件' + '.zip' // 下载后文件名
    ...
    close() {
      ...
      this.departName = this.customerName = this.businessTypeName = this.curingTypeName = ''
      this.visible = false
      this.auditStatus = ''
      this.processList = []
      this.historyList = []
    ...
    exportExcel() {
      ...
      this.json2excel(excelDatas, `工伤事件-${this.model.emName}`, true, 'xlsx', 0, this.fileList.length)
    },
