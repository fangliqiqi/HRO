              ...
              >
                ...
                >
                  ...
                  <detail-list>
                    ...
                    <detail-list-item term="岗位">{{ insuranceInfo.post }}</detail-list-item>
                  </detail-list>
              ...
              >
                ...
                <div v-if="flag == 0 || flag == 1">
                  <a-card :bordered="false">
                    <detail-list>
                      <detail-list-item term="事件发生时间">{{ moment(model.eventOccurrenceTime).format('YYYY-MM-DD') }}</detail-list-item>
                    ...
                    <div v-if="model.eventType == 5">
                      <detail-list>
                        <detail-list-item term="车辆归属单位">{{ model.carOfUnit }}</detail-list-item>
                        <detail-list-item term="车辆保险购买单位">{{ model.carInsuranceUnit }}</detail-list-item>
                      </detail-list>
                      <detail-list>
                        <detail-list-item term="交强险赔付额">{{ model.payStrongInsurance }}</detail-list-item>
                        <detail-list-item term="第三责任险保额">{{ model.thirdPartyLiabilityInsurance }}</detail-list-item>
                      </detail-list>
                      <detail-list>
                        <detail-list-item term="伤者户口归属">{{ (model.woundedHukouAttribution == 0) ? '城镇居民' : '农村户口' }}</detail-list-item>
                        <detail-list-item term="受伤部位">{{ model.injuredArea }}</detail-list-item>
                        <detail-list-item term="受伤程度">{{ model.injuredDegree }}</detail-list-item>
                      </detail-list>
                      <detail-list>
                        <detail-list-item term="第三人身份号">{{ model.thirdPersonIdCard }}</detail-list-item>
                        <detail-list-item term="年龄">{{ model.age }}</detail-list-item>
                      </detail-list>
                      <detail-list>
                        <detail-list-item term="伤者家庭情况">{{ model.injuredFamilyStatus }}</detail-list-item>
                        <detail-list-item term="治疗费（预判）">{{ model.treatmentCosts }}</detail-list-item>
                      </detail-list>
                    </div>
                    <div v-else>
                      <detail-list>
                      ...
                      <detail-list>
                        <detail-list-item term="治疗类型">{{ filterDictText(treatmentTypeOption,model.curingType) }}</detail-list-item>
                      </detail-list>
                      <detail-list>
                ...
                <div v-if="flag == 2">
                  <a-card :bordered="false">
                    ...
                    <detail-list>
                      <detail-list-item term="事件类别">{{ model.eventTypeName }}</detail-list-item>
                    </detail-list>
                    <detail-list
                      layout="vertical"
                      :col="24"
                    >
                      <detail-list-item term="事件说明">{{ model.eventThrough }}</detail-list-item>
                    </detail-list>
                  </a-card>
                  <a-divider orientation="left">方案意见</a-divider>
                  <a-card :bordered="false">
                ...
                <div v-if="flag == 3">
                  <a-card :bordered="false">
                    <detail-list>
                      <detail-list-item term="仲裁时间">{{ moment(model.eventOccurrenceTime).format('YYYY-MM-DD') }}</detail-list-item>
                    </detail-list>
                    <detail-list
                      layout="vertical"
                      :col="24"
  ...
  export default {
    ...
    data() {
      return {
        ...
        validatorRules: {
          ...
          eventOccurrenceTime:{rules: [{ required: true, message: '请选择时间!' }]},
        ...
        url: {
          ...
          submitUrl:'/hrEmergency/tabitrationevent', //提交仲裁
    ...
    methods: {
      ...
      edit(record) {
        ...
        this.insuranceInfo.averageSalary = record.workInjuryEvent.averageSalary
        this.remainParams = {
          ...
          workInjuryId:record.workInjuryEvent.id,
        ...
        //获取附件信息
        httpAction(`${this.url.getAttachUrl}${record.workInjuryEvent.id}?type=${this.flag}`,null,'get').then((res) => {
        ...
        //社保购买地
        this.insuranceInfo.area = record.workInjuryEvent.provinceName + record.workInjuryEvent.cityName + record.workInjuryEvent.townName
        //社保状态
        ...
        //合同期限 历史时间次数
        const param = `${record.workInjuryEvent.empId}?empIdcard=${record.workInjuryEvent.empIdcard}&settleDepartId=${record.workInjuryEvent.settleDomain}`
        httpAction(this.url.getEmpInsurance+param,null,'get').then((res) => {
          if(Number(res.code) === 200 && res.data){
        ...
        switch(this.flag){
          ...
          case 1 :
            ...
            break;
          case 2 :
            this.model = Object.assign({insurance:record.insurance}, record.retiredWorkEvent)
            break;
          case 3 :
        ...
        httpAction(this.url.getinfo+record.workInjuryEvent.empId,null,'get').then((res) => {
          ...
          // this.$nextTick(() => {
          //   console.log(_this.riskForm) // _this.riskForm.fieldsStore.fieldsMeta
          //   _this.riskForm.setFieldsValue(
          //     pick(Object.assign({}, record.workInjuryEvent), 
          //     'workLengthCustomerYear', 'workLengthCustomerMonth','workLengthWxMonth','workLengthWxYear')
          //   )
          // })
        })
      },
      ...
      handleOk() {
        ...
        }else if(Number(this.flag) === 1){
          formData.nonWorkId = formData.workInjuryId
          delete formData.workInjuryId
          params.abitrationEvent = formData
        }else if(Number(this.flag) === 2){
          formData.retiredId = formData.workInjuryId
          delete formData.workInjuryId
          params.abitrationEvent = formData
        }else if(Number(this.flag) === 3){ //转诉讼
          formData.abitrationId = formData.workInjuryId
          delete formData.workInjuryId
      ...
      history(){
        if(this.insuranceInfo.record){
          ...
          this.$refs.HistoryEventModalForm.customerTreeData = this.customerTreeData
