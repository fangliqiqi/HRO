        ...
        <a-row style="margin-bottom:15px">
          ...
          >
            <a-switch
              ...
              :checked="handleWorkInjuryStatus"
              :disabled="handleWorkInjuryStatus"
              @change="handleStatus"
            />
          </a-col>
        </a-row>
              ...
              <a-col :span="24">
                ...
                >
                  ...
                  >
                    ...
                    <a-select-option
                      v-for="(item, index) in socialClaimsFeeTypeDicts"
                      :key="index"
                      :value="item.value"
...
export default {
  ...
  data() {
    return {
      ...
      handleWorkInjuryStatus:false, //工伤办理状态
      handleVisible: false,
  ...
  methods: {
    ...
    edit(record) {
      this.id = record.id
      this.handleWorkInjuryStatus = (record.handleWorkInjuryStatus == 3) ? true : false
      
      this.visible = true
      if(record.lastMaterialSubDate){
        this.disableSet = true
        this.$nextTick(() => {
          this.deadlineForm.setFieldsValue(pick({lastMaterialSubDate:moment(record.lastMaterialSubDate)}, 'lastMaterialSubDate'))
        })
      }
    },
    //设置截止时间
    submitDeadline(){
      this.deadlineForm.validateFields((err, values) => {
        if (!err) {
          let lastMaterialSubDate = moment(values.lastMaterialSubDate).format("YYYY-MM-DD")
          httpAction(this.url.deadlineUrl+'?id='+this.id+'&lastMaterialSubDate='+lastMaterialSubDate,null,'post').then((res) => {
            if(res.code == 200 ){
              this.disableSet = true
    ...
    submitCostRecord(){
      this.costForm.validateFields((err, values) => {
        if(!err){
          let params = []
          let total = 0
          for (let [index,val] of values.costExpenditureType.entries()){
              params.push({
                workInjuryId:this.id,
                costExpenditureType:val,
                costExpenditureFee:values.costExpenditureFee[index],
                payMainType:0
              })
              total = parseFloat(total) + parseFloat(values.costExpenditureFee[index])
          }
    ...
    handleSucess(){
      this.handleForm.validateFields((err, values) => {
        if (!err) {
          ...
          httpAction(`${this.url.endInjury}0/${this.id}?disabilityLevel=${values.disabilityLevel|| '' }`,null,'post').then((res)=>{
            if(res.code == 200){
              this.$message.success('操作成功')
              this.handleWorkInjuryStatus = true
              this.cancelEvent()
              this.handleCancel()
            }else{
              this.$message.error(res.msg)
            ...
            this.handleWorkInjuryStatus = false
