        ...
        >
          <a-row class="mbot">
            ...
            >
              ...
              {{ rowEmpoyee.empAge ? (rowEmpoyee.empAge+'岁') : '' }}
            </a-col>
            <a-col
              :span="3"
              :offset="1"
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          // customRender: text => {
          //   return this.settleDomainOptions[text] ? this.settleDomainOptions[text]['departName'] : ''
          // }
        },
        {
          title: '手机号',
      ...
      validatorRules: {
        ...
        carOfUnit: { rules: [{ required: true, message: '请输入车辆归属单位!' },{max:50,message:'车辆归属单位最多50个字符!'}] },
        carInsuranceUnit: { rules: [{ required: true, message: '请输入车辆保险购买单位!' },{max:50,message:'车辆保险购买单位最多50个字符!'}] },
        payStrongInsurance: { rules: [{ required: true, pattern:/(?!^0*(\.0{1,2})?$)^\d{1,13}(\.\d{1,2})?$/,message: '交强险赔付额格式不正确!' }] },
        thirdPartyLiabilityInsurance: { rules: [{ required: true,  pattern:/(?!^0*(\.0{1,2})?$)^\d{1,13}(\.\d{1,2})?$/,message: '第三责任险保额格式不正确!' }] },
        woundedHukouAttribution: { rules: [{ required: true, message: '请选择伤者户口归属!' }] },
        injuredArea: { rules: [{ required: true, message: '请输入受伤部位!' },{max:50,message:'受伤部位最多50个字符!'}] },
        injuredDegree: { rules: [{ required: true, message: '请输入受伤程度!' },{max:50,message:'受伤程度最多50个字符!'}] },
        thirdPersonIdCard: { rules: [{ required: true, message: '请输入第三人身份号!' },{validator:(rule, val, callback)=>{
          if(val){
        ...
        injuredFamilyStatus: { rules: [{ required: true, message: '请输入伤者家庭情况!' },{max:50,message:'家庭情况最多50个字符!'}] },
        treatmentCosts: { rules: [{ required: true,pattern:/(?!^0*(\.0{1,2})?$)^\d{1,13}(\.\d{1,2})?$/, message: '治疗费格式不正确!' }] },
      },
      labelCol: {
        xs: { span: 24 },
        sm: { span: 5 }
      ...
      url: {
        ...
        submitApplyUrl: '/hrEmergency/tworkinjuryevent', //工伤申报
        empValidator:'/hrEmergency/tworkinjuryevent/event/'
      }
    }
  },
  methods: {
    ...
    onSelect(value){
      this.isCuringType = (value == 1) ? false : true
    },
    selectType(value){
      this.traffic = (value == 5) ? true : false
    },
    handleSelect(val){
    ...
    next() {
      ...
      if (this.current === 1) {
        ...
        this.empForm.validateFields((err, values) => {
          if (!err) {
            values.empAge = this.selectionRows.empAge || 0
    ...
    history(){
      if(this.insuranceInfo.record){
        this.$refs.HistoryEventModalForm.flag = 0
    ...
    handleOk() {
      ...
      const formData = Object.assign(this.model, this.allModel, {
        ...
        if(Number(result.code) === 200){
          if(result.data){
            ...
            httpAction(`${this.url.getCustomerById}?cusId=${this.rowEmpoyee.belongUnit}&settleDomainId=${this.rowEmpoyee.settleDomain}`,null,'get').then(res1=>{
              if(Number(res1.code) === 200 && res1.data){
                ...
                httpAction(`${this.url.getEmpInsurance}${this.rowEmpoyee.id}?type=1`, null, 'get').then(res => {
                  if (Number(res.code) === 200 && res.data) {
                    ...
                    for (const item of this.insuranceInfo.insuranceArr) {
            ...
            //     '?cusId=' +
            //     this.rowEmpoyee.belongUnit +
            //     '&settleDomainId=' +
            //     this.rowEmpoyee.settleDomain,
            //   null,
            //   'get'
            // ).then(res => {
            //   if(Number(res.code) === 200 && res.data){
            //     this.rowEmpoyee.customerName = res.data.cusName //客户单位 this.businessTypeDicts
            //     this.rowEmpoyee.businessType = res.data.businessType
            //     this.rowEmpoyee.businessTypeName = filterDict(this.businessTypeDicts, res.data.businessType)
            //   }else{
