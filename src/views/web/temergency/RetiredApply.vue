  ...
  <a-card :bordered="false">
    ...
    <div class="table-operator">
      <a-button
        type="primary"
        icon="plus"
        v-has="'wxhr:tretiredworkevent_add'"
        @click="handleAdd"
      >退工申报</a-button>
      <a-dropdown v-has="'wxhr:tretiredworkevent_add'">
        <a-menu slot="overlay">
          <a-menu-item v-has="'wxhr:tretiredworkevent_add'">
            <a-upload
              name="file"
          ...
          <a-menu-item @click="handleDownloadTemplate(url.templateUrl)">
            <a-icon type="download" />模板下载</a-menu-item>
        </a-menu>
        <a-button style="margin-left: 8px">批量退工操作
          <a-icon type="down" />
        </a-button>
      </a-dropdown>
    </div>
    ...
    <div>
      ...
      >
        ...
        >
          ...
          <a-divider
            type="vertical"
            v-has="'wxhr:tretiredworkevent_edit'"
          ...
          <a-divider
            type="vertical"
            v-has="'wxhr:tretiredworkevent_batch_commit'"
          ...
          <a-divider
            type="vertical"
            v-has="'wxhr:tretiredworkevent_del'"
            v-if="record.auditStatus == 0 || record.auditStatus == 3"
          />
          <a-popconfirm
            title="确定删除吗?"
    ...
    ></retired-apply-modal>
    <retired-audit-modal
      ref="RetiredAuditModalForm"
      @ok="modalFormOk"
    ></retired-audit-modal>
    <retired-apply-edit-modal
      ref="RetiredApplyEditModalForm"
      @ok="modalFormOk"
    ></retired-apply-edit-modal>
...
<script>
  ...
  import RetiredApplyModal from './modules/RetiredApplyModal'
  import RetiredAuditModal from './modules/RetiredAuditModal'
  import RetiredApplyEditModal from './modules/RetiredApplyEditModal'
  ...
  export default {
    ...
    data() {
      return {
        ...
        url: {
          list: '/hrEmergency/tretiredworkevent/page',
          delete:'/hrEmergency/tretiredworkevent/',
          queryAllInfoById: '/hrEmergency/tretiredworkevent/vo/',
          againCommit:'/hrEmergency/tretiredworkevent/commitAgain/', //再次提交
          getBelongOrg:'/admin/organizationinfo/getOrgToTree',
          getTreeData: '/hrBase/tcustomerinfo/ValueTree',
          importExcelBatchAddUrl:'hrEmergency/tretiredworkevent/bactchSaveByExcel', //批量导入
          templateUrl:'/templates/批量退工模板.xlsx' //模板
        }
      }
    },
    computed:{
    ...
    methods: {
      ...
      // 新建
      handleAdd: function() {
        this.$refs.modalForm.title = '退工申报'
        this.$refs.modalForm.url.submitApplyUrl = '/hrEmergency/tretiredworkevent'
      ...
      handleDetail(record){
        ...
        this.$refs.RetiredAuditModalForm.auditStatus = '状态：'+filterDictText(this.auditStatusOption, record.auditStatus)
        ...
        this.$refs.RetiredAuditModalForm.eventTypeOptions = this.eventTypeOptions

        httpAction(this.url.queryAllInfoById+record.id,null,'get').then((res) => {
          if(res.code == 200){
            this.$refs.RetiredAuditModalForm.edit(res.data)
          }
        })
      ...
      handleEdit: function(record) {
        this.$refs.RetiredApplyEditModalForm.flag = 2 //标记
        this.$refs.RetiredApplyEditModalForm.title = '退工事件_编辑'
        this.$refs.RetiredApplyEditModalForm.eventTypeOptions = this.eventTypeOptions
        this.$refs.RetiredApplyEditModalForm.settleDomainOptions = this.settleDomainOptions
        this.$refs.RetiredApplyEditModalForm.businessTypeDicts = this.businessTypeDicts
        if(record.auditStatus == 2){
          this.$refs.InjuryAttachModalForm.flag = 2
          this.$refs.InjuryAttachModalForm.edit(record)
        }else{
          httpAction(this.url.queryAllInfoById+record.id,null,'get').then((res) => {
            if(res.code == 200){
              // res.data.workInjuryEvent = res.data.retiredWorkEvent
              this.$refs.RetiredApplyEditModalForm.edit(res.data)
            }
          })
        }
      },
      ...
      handleSubmit: function(record) {
        ...
        if(record.auditStatus == 2){
          ...
          this.$refs.InjuryApplySubmitModalForm.isShowGs = false
          this.$refs.InjuryApplySubmitModalForm.isShowSx = false
          this.$refs.InjuryApplySubmitModalForm.isHandle = false
          this.$refs.InjuryApplySubmitModalForm.idRetired = true
          this.$refs.InjuryApplySubmitModalForm.permissionStr = {
            audit:'wxhr:tretiredworkevent_audit_pass',
            handle:'wxhr:tretiredworkevent_end'
          }
          this.$refs.InjuryApplySubmitModalForm.url.submitUrl = '/hrEmergency/tretiredworkevent/batch/commit'
