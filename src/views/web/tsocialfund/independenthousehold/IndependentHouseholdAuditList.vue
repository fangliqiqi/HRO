      ...
      >
        ...
        >
          ...
          <a-divider
            ...
            v-has="'wxhr:tforecastoneselfform_audit'"
          />
          <a
            v-if="record.status==1"
...
<script>
  ...
  export default {
    name: 'IndependentHouseholdAuditList',
    ...
    data() {
      return {
        ...
        url: {
          list: 'hrSocial/tforecastoneselfform/page?status=1', // 列表
    ...
    methods: {
      ...
      handleAudit: function(record) {
        this.$refs.billDetailModal.auditStatus = true
        this.$refs.billDetailModal.title = '结算单详情'
        this.$refs.billDetailModal.invoiceTypeOptions = this.invoiceTypeOptions
        httpAction(`${this.url.queryAllInfoById}?id=${record.id}`, {}, 'GET').then((res) => {
          if (Number(res.code) === 200 && res.data) {
