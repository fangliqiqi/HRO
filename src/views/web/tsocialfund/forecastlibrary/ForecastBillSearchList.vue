    ...
    <div>
      ...
      >
        ...
        >
          ...
          <a-divider type="vertical" v-if="record.status==1" v-has="'wxhr:tforecastform_back'"/>
          <a @click="handleBack(record)" v-if="record.status==1" v-has="'wxhr:tforecastform_back'">管理员退回</a>
        </span>
      </a-table>
    </div>
    <!-- table区域-end -->
  ...
  export default {
    ...
    data() {
      return {
        ...
        url: {
          list: 'hrSocial/tforecastform/page', // 列表
          ...
          back:'hrSocial/tforecastform/forecastFormBack'
        },
      }
    },
    created() {
    ...
    methods: {
      ...
      handleBack(record){
        ...
        this.$confirm({
          title: '是否退回?',
          onOk() {
            httpAction(`${_this.url.back}?formId=${record.id}`,null,'post').then(res=>{
              if(Number(res.code) === 200){
