  ...
  <a-card :bordered="false">
            ...
            >
              ...
              >
                <a-select
                  ...
                  :dropdownMatchSelectWidth="false"
                >
                  <a-select-option value="0">待办理</a-select-option>
    ...
    <div class="table-operator">
      <a-button
        type="primary"
        icon="download"
        @click="handleDoexportSocialRecordRoster"
        v-has="'wxhr:tdispatchinfo_doexportSocialRecordRoster'"
      >导出花名册</a-button>
      <!-- 批量办理社保 -->
      <a-button
        ...
        v-has="'wxhr:tdispatchinfo_addApplyHandle'"
      >批量办理社保</a-button>
    </div>
    <div
      ...
      >
        ...
        >
          ...
          <a-divider
            type="vertical"
            v-has="'wxhr:tdispatchinfo_addApplyHandle'"
            v-if="record.socialHandleStatus == '0' || record.socialHandleStatus == '4'"
          />
          <a
            v-if="record.socialHandleStatus == '0' || record.socialHandleStatus == '4'"
            @click="handleAddApply(record.id,record)"
            v-has="'wxhr:tdispatchinfo_addApplyHandle'"
          >办理</a>
        </span>
      </a-table>
...
<script>
  ...
  export default {
    name: 'SocialSecurityManagementList',
      ...
      return {
        ...
        columns: [
          ...
          },
          {
            ...
            customRender:(text) => {
              const status = this.socialHandleStatusOption[text]
              return <a-tooltip placement="topLeft" title={status}>{status}</a-tooltip>
            },
          },
          {
            width: 80,
        ...
        url: {
          list: 'hrSocial/tdispatchinfo/pageOfOperator?deleteFlag=0&status=2&typeSub=0&searchType=2',
          ...
          doexportSocialRecordRoster: 'hrSocial/tdispatchinfo/doexportSocialRecordRoster?searchType=2',  // 导出花名册
          auditProcessUrl: 'hrSocial/tauditinfo/getAuditInfoByIdAndType',  // 流程进展明细
          getExportField: 'hrSocial/tdispatchinfo/getSocialHandleExportVoFieldName',
        },
      }
    },
    created() {
    ...
    methods: {
      ...
      getCheckboxProps(){
        return {props:{
          // disabled:record.socialHandleStatus !== '0'
        }}
      },
      // 详情
          ...
          if (this.fundPayPointOptions.length > 0 && this.empTypeOptions.length > 0 && Object.keys(this.settleDomainOptions).length > 0 && (Array.isArray(Vue.ls.get('glob_area')) && Vue.ls.get('glob_area').length > 0)) {
            httpAction(`${this.url.queryAllInfoById}?id=${record.id}`, '', 'GET').then((res) => {
              if (Number(res.code) === 200 && res.data) {
                this.$refs.dispatchAddApplicationDetailModal.employeeContractInfo = res.data.dispatchInfo //工时制
                res.data.fund = null
                // 获取社保户
                getHouseHold({id: res.data.socialInfo.socialHousehold}).then((socialHousehold) => {
                  if (socialHousehold.code === 200 && socialHousehold.data.length > 0) {
                    this.$refs.dispatchAddApplicationDetailModal.houseHolds = socialHousehold.data
      ...
      // 办理
      handleAddApply: function(id,record) {
        let ids = '';
        let flag = false;
        const res = groupByField(this.selectionRows,'type');
        if (String(id) === 'false') {
          if (this.selectedRowKeys.length <= 0) {
            ...
            // 判断是否同时存在派增派减
            if(arr.length > 1){
              this.$message.warning('请选择全为派增或派减的数据!');
              return;
            }else{
              flag = true
              // 判断是否全为派减
              ...
              this.$refs.dispatchAddApplyModal.title = '批量办理'
          ...
          ids = id
          this.$refs.dispatchAddApplyModal.title = '社保办理'
        }
        this.$refs.dispatchAddApplyModal.ids = ids
        this.$refs.dispatchAddApplyModal.typeSub = 0  // 社保办理
        this.$refs.dispatchAddApplyModal.add(record,flag)
      },
      // 导出花名册
      handleDoexportSocialRecordRoster: function() {
        httpAction(this.url.getExportField,{},'GET').then((res) => {
          if(Number(res.code) === 200){
            this.exportFields = res.data
            let requestExportXlsUrl = this.url.doexportSocialRecordRoster
            ...
            this.$refs.socialFundExportModal.title = '导出花名册'
