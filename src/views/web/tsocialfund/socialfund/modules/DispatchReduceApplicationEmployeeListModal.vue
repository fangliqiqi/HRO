<template>
  ...
  >
    ...
    <dispatch-reduce-application-modal
      ...
      @ok="dispatchReduceApplicationModalFormOk"
    ></dispatch-reduce-application-modal>
  </a-modal>
</template>
...
import DispatchReduceApplicationModal from './DispatchReduceApplicationModal'

export default {
  name: 'DispatchReduceApplicationEmployeeListModal',
  components: {
  ...
  data() {
    return {
      ...
      url: {
        list: 'hrBase/temployeeinfo/pageForDispatch?deleteFlag=0&searchType=1', // 员工信息列表[searchType:0可派增，1可派减]
  ...
  methods: {
    ...
    openDispatchApplicationModal(record) {
      ...
      this.$refs.dispatchReduceApplicationModal.title = '减少派单'
      this.$refs.dispatchReduceApplicationModal.empTypeBelongs = this.empTypeBelongs
      this.$refs.dispatchReduceApplicationModal.reduceReasonBelongs = this.reduceReasonBelongs
      this.$refs.dispatchReduceApplicationModal.settleDomainBelongs = this.settleDomainBelongs
      httpAction(this.url.getEmployeeStatusById + '?empId=' + record.id, '', 'GET').then((res) => {
        if (Number(res.code) === 200 && res.data) {
          this.$refs.dispatchReduceApplicationModal.employeeStatus = res.data
          console.log(record,11)
          this.$refs.dispatchReduceApplicationModal.edit(record)
        } else {
    ...
    // 新建弹框关闭
    dispatchReduceApplicationModalFormOk() {
      this.$emit('ok')
    },
    // 表单关闭
    handleCancel() {
