<template>
  <a-card :bordered="false">
    ...
    <!-- 表单区域 -->
    <!-- 审核 -->
    <financial-bills-audit-modal 
      ref="financialBillsAuditModal" 
      @ok="modalFormOk"
    ></financial-bills-audit-modal>
    <!-- 编辑、详情等-->
...
<script>
  ...
  import FinancialBillsAuditModal from './modules/FinancialBillsAuditModal' //审核弹窗

  export default {
    name: 'FinancialBillsSettleAuditList',//财务账单审核-全国运营中心
    mixins: [JeecgListMixin],
    components: {
      SelectPage,
      FinancialBillsModal,
      FinancialBillsAuditModal
    },
    data() {
      return {
        ...
        url: {
          list: 'hrSocial/tfinancebill/getTFinanceBillFirstAudit', // 列表
          exportUrl:'/hrSocial/tfinancebill/getTFinanceBillFirstAuditList' //导出
        ...
        if(this.selectedRowKeys.length>0){
         ...
         httpAction(`${this.url.exportUrl}?${qs.stringify(params)}`,null,'get').then(res=>{
           if(res.code==200){
            ...
            this.json2excel(excelDatas, '财务账单审核', true, "xlsx")
    ...
    formatJson(filterVal, jsonData) {
      ...
      // 审核
      handleAudit(record){
        this.$refs.financialBillsAuditModal.show(record)
      },
      // 详情
      handleDetail(record){
