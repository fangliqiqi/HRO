  ...
  <a-card :bordered="false">
    ...
    <div class="table-operator">
      <a-button
        ...
        icon="plus"
        @click="handleAdd"
        v-has="'wxhr:tsocialaprovalform_add'"
      >生成核准表</a-button>
    </div>
      ...
      >
        ...
        >
          ...
          <a-divider
            ...
            v-has="'wxhr:tsocialaprovalform_submit'"
          />
          <a
          ...
          <a-divider
            type="vertical"
            v-if="record.status==0 || record.status==3 || record.status==4"
            v-has="'wxhr:tsocialaprovalform_del'"
          />
          <a-popconfirm
            title="确定删除吗?"
            v-if="record.status==0 || record.status==3 || record.status==4"
    ...
    <!-- 表单区域 -->
    <social-approval-apply-modal
      ref="socialApprovalApplyModal"
      @ok="modalFormOk"
    ></social-approval-apply-modal>
    <social-approval-apply-detail-modal
      ref="socialApprovalApplyDetailModal"
      @ok="modalFormOk"
    ></social-approval-apply-detail-modal>
...
<script>
  ...
  import SocialApprovalApplyModal from './modules/SocialApprovalApplyModal'
  import SocialApprovalApplyDetailModal from './modules/SocialApprovalApplyDetailModal'

  export default {
    name: 'SocialApprovalApplyList',
    mixins: [JeecgListMixin],
    components: {
      ErrorModal,
      SocialApprovalApplyModal,
      SocialApprovalApplyDetailModal,
    },
    data() {
      return {
        description: '社保核准申请',
        ...
        url: {
          list: 'hrSocial/tsocialaprovalform/pageOfUser',
          delete: 'hrSocial/tsocialaprovalform/',
    ...
    created() {
      ...
      // 获取权限的用户
      httpAction('admin/user/findUserBypPrmission?permissionStr=wxhr:tsocialaprovalform_outAccount', '', 'GET').then((res) => {
        if (Number(res.code) === 200) {
    ...
    methods: {
      // 生成核准表
      handleAdd: function() {
        this.$refs.socialApprovalApplyModal.title = '生成社保核准表'
        this.$refs.socialApprovalApplyModal.organizationBelongs = this.organizationOptions
        this.$refs.socialApprovalApplyModal.areaTrees =  Vue.ls.get('glob_area')
        if (this.organizationOptions.length > 0 && (Array.isArray(Vue.ls.get('glob_area')) && Vue.ls.get('glob_area').length > 0)) {
          this.$refs.socialApprovalApplyModal.add()
        } else {
      ...
      handleEdit: function(record) {
        this.$refs.socialApprovalApplyDetailModal.id = record.id
        this.$refs.socialApprovalApplyDetailModal.title = '社保核准表提交'
        this.$refs.socialApprovalApplyDetailModal.isVerify = true
        this.$refs.socialApprovalApplyDetailModal.payTypeBelongs = this.payTypeOptions
        this.$refs.socialApprovalApplyDetailModal.nextUserIdBelongs = this.nextUserIdOptions
        this.$refs.socialApprovalApplyDetailModal.areaTrees = this.areaTrees
        if (this.payTypeOptions.length > 0) {
          httpAction(`${this.url.queryAllInfoById}?id=${record.id}`, '', 'GET', 10000 * 20).then((res) => {
            if (Number(res.code) === 200 && res.data) {
              this.$refs.socialApprovalApplyDetailModal.edit(res.data)
            } else {
      ...
      handleDetail: function(record) {
        this.$refs.socialApprovalApplyDetailModal.title = '社保核准表详情'
        this.$refs.socialApprovalApplyDetailModal.payTypeBelongs = this.payTypeOptions
        if (this.payTypeOptions.length > 0) {
