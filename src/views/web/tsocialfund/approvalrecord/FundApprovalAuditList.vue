      ...
      >
        ...
        >
          ...
          <a-divider
            ...
            v-has="'wxhr:tProvidentaprovalform_audit'"
          />
          <a
  ...
  export default {
    ...
    data() {
      return {
        ...
        url: {
          list: 'hrSocial/tprovidentaprovalform/page?status=5',
    ...
    methods: {
      ...
      handleEdit: function(record) {
        ...
        this.$refs.fundApprovalApplyDetailModal.title = '公积金核准表审核'
        this.$refs.fundApprovalApplyDetailModal.isAudit = true
      ...
      handleDetail: function(record) {
        ...
        this.$refs.fundApprovalApplyDetailModal.payTypeBelongs = this.payTypeOptions
        if (this.payTypeOptions.length > 0) {
          const requestUrl = `${this.url.queryAllInfoById}?id=${record.id}`
          httpAction(requestUrl, '', 'GET',10000 * 20).then((res) => {
            if (Number(res.code) === 200 && res.data) {
              this.$refs.fundApprovalApplyDetailModal.edit(res.data)
            } else {
              this.$message.error('数据请求失败！')
