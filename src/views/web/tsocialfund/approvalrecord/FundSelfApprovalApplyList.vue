...
export default {
  name: 'FundSelfApprovalApplyList',
  ...
  data() {
    return {
      ...
      url: {
        list: 'hrSocial/tprovidentaprovalform/page',
  ...
  created() {
    ...
    // 获取权限的用户
    httpAction('admin/user/findUserBypPrmission?permissionStr=wxhr:tProvidentaprovalform_outAccount', '', 'GET').then(
      res => {
        if (res.code === 200) {
  ...
  methods: {
    ...
    handleDetail: function(record) {
      ...
      this.$refs.fundApprovalApplyDetailModal.payTypeBelongs = this.payTypeOptions
      if (this.payTypeOptions.length > 0) {
        const requedtUrl = `${this.url.queryAllInfoById}?id=${record.id}`
        httpAction(requedtUrl, '', 'GET').then(res => {
          if (Number(res.code) === 200 && res.data) {
            this.$refs.fundApprovalApplyDetailModal.edit(res.data)
          } else {
            this.$message.error('数据请求失败！')
