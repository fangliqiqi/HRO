  ...
  >
    ...
    <a-spin :spinning="confirmLoading">
      ...
      >
        ...
        <a-row>
          <a-col :span="8">
            <a-form-item label="学校" class="stepFormText">
              <a-input
                placeholder="请填写学校名称"
                v-decorator="['school', validatorRules.school]"
                style="width: 154px"
              />
            </a-form-item>
          </a-col>
          <a-col :span="8">
            ...
            >
              <a-input
                placeholder="请填写院系名称"
                v-decorator="['collageSystem']"
                style="width: 154px"
              />
            </a-form-item>
          </a-col>
          <a-col :span="8">
            <a-form-item label="专业" class="stepFormText">
              <a-input
                placeholder="请填写学历名称"
                v-decorator="['bachelorScience', {rules: [{ required: true, message: '请填写学历名称！' }]}]"
                style="width: 154px"
              />
            </a-form-item>
          </a-col>
        ...
        <a-row>
          <a-col :span="8">
            ...
            >
              <a-select
                ...
                placeholder="请选择学历"
                v-decorator="['educationName', {rules: [{ required: true, message: '请选择学历级别！' }]}]"
                :dropdownMatchSelectWidth="false"
                style="width: 154px"
              >
                <a-select-option
                  v-for="(item, index) in educationBelongs"
                  :key="index"
                  :value="item.value"
          ...
          <a-col :span="8">
            ...
            >
              ...
              >
                <a-select-option
                  v-for="(item, index) in educationTypeBelongs"
                  :key="index"
                  :value="item.value"
                >{{ item.label }}</a-select-option>
              </a-select>
          ...
          <a-col :span="8">
            <a-form-item label="最高学历" class="stepFormText">
              <a-radio-group
                class="fontiframe"
                name="radioGroup"
                v-decorator="['hightIdentification', {initialValue: '0', rules: [{ required: true, message: '请选择！' }]}]"
              >
                <a-radio
                  class="radioGroup"
                  v-for="(value, key) in hightIdentificationBelongs"
                  :key="key"
                  :value="key"
        ...
        <a-row>
          <a-col :span="8">
            <a-form-item label="入学时间">
              <a-date-picker
                ...
                placeholder="请选择入学时间"
                v-decorator="['entryDate', {initialValue: !model.entryDate ? null : moment(model.entryDate, dateFormat), rules:[{required: true, message: '请选择入学日期！'}]}]"
                style="width: 154px"
              />
            </a-form-item>
          </a-col>
          <a-col :span="8">
            <a-form-item label="毕业时间">
              <a-date-picker
                :disabledDate="disabledGradutionDate"
                placeholder="请选择毕业时间"
                v-decorator="['gradutionDate', {initialValue: !model.gradutionDate ? null : moment(model.gradutionDate, dateFormat), rules:[{required: true, message: '请选择结业日期！'}]}]"
                style="width: 154px"
              />
            </a-form-item>
          </a-col>
          <a-col :span="8">
            ...
            >
              <a-select
                ...
                optionFilterProp="children"
                placeholder="请选择学制类型"
                v-decorator="['educationSystem']"
                :dropdownMatchSelectWidth="false"
                style="width: 154px"
              >
                <a-select-option
                  v-for="(item, index) in educationSystemBelongs"
                  :key="index"
                  :value="item.value"
                >{{ item.label }}</a-select-option>
              </a-select>
  ...
  export default {
    ...
    data() {
      return {
        ...
        hightIdentificationBelongs: {}, // 最高学历
        ...
        validatorRules: {
          ...
          school: { rules: [{ required: true, message: '请填写学校名称！' }] },
        },
        url: {
          add: 'hrBase/tempeducation',
          edit: 'hrBase/tempeducation',
          importUrl:'hrBase/fileUpload/ossUploadFile?type=0',
          deleteAtta: 'hrBase/fileUpload/ossFileDelete/',
          updateaDeleteAtta: 'hrBase/tempeducation/deleteEducationFile',
        },
      }
    },
    computed: {
    ...
    methods: {
      ...
      edit(record) {
        ...
        this.visible = true
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model, 'school', 'collageSystem', 'bachelorScience', 'educationName', 'type', 'hightIdentification', 'educationSystem', 'certificationName', 'remark'))
        })
        // 编辑时候附件显示
        if (this.id) {
      ...
      handleOk() {
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            // 入学日期格式转换
            values.entryDate = values.entryDate ? values.entryDate.format(this.dateFormat) : null
            // 结业日期格式转换
            values.gradutionDate = values.gradutionDate ? values.gradutionDate.format(this.dateFormat) : null
            ...
            if (!this.model.id) {
              ...
              formData.educationAttaIds = uids
            } else {
              // 编辑的时候的时候// 附件
              ...
              formData.educationAttaIds = uids
              // 删除表单不需要的字段
              delete formData.educationAttaList
      ...
      handleRemove(file) {
        ...
        if (this.id) {  // 编辑的时候
          ...
          if (editIndex !== -1) {  // 找到了（说明是新增加的图片）
            ...
            reqUrl = `${this.url.updateaDeleteAtta}?educationId=${this.id}&id=${file.uid}`
            method = 'post'
          }
        } else {  //添加的时候
          reqUrl = this.url.deleteAtta + file.uid
      ...
      // 入学日期
      entryDateChange() {
        this.model.gradutionDate = null
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model, 'gradutionDate'))
        })
      },
      // 入学日期
      disabledEntryDate(current) {
        return current && current > moment().endOf('day')
      },
      // 结业日期
      disabledGradutionDate(current) {
        const entryDate = this.form.getFieldValue('entryDate')
        if (!current || !entryDate) {
          return current
        } else {
          return entryDate.valueOf() > current.valueOf()
        }
      },
      // 员工选择改变
