  ...
  <a-card :bordered="false">
          ...
          <template v-if="toggleSearchStatus">
            ...
            >
              <a-form-item label="结算月份">
                <a-month-picker placeholder="结算月份" v-model="queryParam.settleMonth_extra" @change="onChangeMonth" />
              </a-form-item>
            </a-col>
            <a-col
              :md="6"
    ...
    <div class="table-operator">
      <a-button
        ...
        type="primary"
        icon="plus"
        v-has="'wxhr:tbusinessoutsourcingmain_add'"
      >新建业务</a-button>
    </div>
    ...
    >
      ...
      >
        ...
        <a-divider
          type="vertical"
          v-if="record.settleBillVo && record.auditFlag==0"
          v-has="'wxhr:tbusinessoutsourcingmain_submit'"
        />
        <a
          v-if="record.settleBillVo && record.auditFlag==0"
        ...
        <a-divider
          type="vertical"
          v-if="record.auditFlag==0 && record.settleBillVo"
        />
        <a
          v-if="record.auditFlag==0 && record.settleBillVo"
          @click="makeInvoice(record)"
        >开票</a>
        <a-divider
          type="vertical"
          v-if="record.auditFlag==0 || record.auditFlag==3"
          v-has="'wxhr:tsettlebill_add'"
        />
        <a
          v-if="record.auditFlag==0 || record.auditFlag==3"
          v-has="'wxhr:tsettlebill_add'"
          @click="makeSettleBill(record)"
        >结算单制作</a>
        <a-divider
          type="vertical"
          v-if="record.settleBillVo && (record.auditFlag==0 || record.auditFlag==3)"
          v-has="'wxhr:tsettlebillinner_add'"
        />
        <a
          v-if="record.settleBillVo && (record.auditFlag==0 || record.auditFlag==3)"
          v-has="'wxhr:tsettlebillinner_add'"
          @click="makeIncomeBelong(record)"
        >收入归属</a>
        <a-divider
          type="vertical"
          v-if="record.auditFlag==0 || record.auditFlag==3"
        />
        <a-dropdown v-if="record.auditFlag==0 || record.auditFlag==3">
          ...
          <a-menu slot="overlay">
            <a-menu-item>
              <a
                ...
                v-has="'wxhr:tbusinessoutsourcingmain_edit'"
              >编辑</a>
            </a-menu-item>
            <a-menu-item v-has="'wxhr:tbusinessoutsourcingmain_del'">
              <a-popconfirm
                title="确定删除吗?"
    ...
    <!-- 表单区域 -->
    <business-out-sourcing-new-module
      ref="businessNewModal"
      @ok="businessNewModalOk"
    ></business-out-sourcing-new-module>
    <business-out-sourcing-audit-module
      ref="auditModal"
      @ok="auditModalOk"
    ></business-out-sourcing-audit-module>
    <inner-settle-bill-module
      ref="innerSettleBillModal"
      @ok="modalFormOk"
    ></inner-settle-bill-module>
    <income-belong-module ref="incomeBelongModal"></income-belong-module>
    <all-detail-module ref="allDetailModal"></all-detail-module>
    <invoice-info-module ref="InvoiceInfoModuleFrom"></invoice-info-module>
  </a-card>
</template>
...
import BusinessOutSourcingNewModule from './modules/BusinessOutSourcingNewModule'
import { filterWxSysUserDict } from '@/utils/customDict'
import BusinessOutSourcingAuditModule from './modules/BusinessOutSourcingAuditModule'
import InnerSettleBillModule from './modules/InnerSettleBillModule'
import IncomeBelongModule from './modules/IncomeBelongModule'
import AllDetailModule from './modules/AllDetailModule'
import InvoiceInfoModule from './modules/InvoiceInfoModule'

export default {
  name: 'BusinessOutSroucingList',
  mixins: [JeecgListMixin],
  components: {
    AllDetailModule,
    IncomeBelongModule,
    InnerSettleBillModule,
    BusinessOutSourcingAuditModule,
    BusinessOutSourcingNewModule,
    InvoiceInfoModule
  },
  data() {
    return {
      ...
      url: {
        list: '/hrOneTime/tbusinessoutsourcingmain/page',
        ...
        getOne: '/hrOneTime/tbusinessoutsourcingmain/',
  ...
  methods: {
    ...
    // 开票
    makeInvoice(record){
      this.$refs.InvoiceInfoModuleFrom.show(record)
    },
    // 数据加载中提示
    dataLoadingTip(){
    ...
    add() {
      this.$refs.businessNewModal.title = "新建一次性结算业务"
      if (this.sysUsers.length > 0 && Object.keys(this.settleDomainOptions).length > 0) {
        this.$refs.businessNewModal.sysUsers = this.sysUsers
        this.$refs.businessNewModal.settleDomainOptions = this.settleDomainOptions
        this.$refs.businessNewModal.add()
      } else {
        this.dataLoadingTip()
      }
    },
    handleEdit(record) {
      this.$refs.businessNewModal.title = "编辑一次性结算业务"
      if (this.sysUsers.length > 0 && Object.keys(this.settleDomainOptions).length > 0) {
        this.$refs.businessNewModal.sysUsers = this.sysUsers
        this.$refs.businessNewModal.settleDomainOptions = this.settleDomainOptions
        httpAction(this.url.findAllDetail + record.id, null, 'GET').then((res) => {
          if(Number(res.code) === 200 && res.data) {
            this.$refs.businessNewModal.resList.push({
              id:res.data.settleDomainId,
              title:res.data.departName,
            })
            this.$refs.businessNewModal.edit(res.data)
          } else {
    ...
    makeSettleBill(record) {
      this.$refs.innerSettleBillModal.title = "结算单制作";
      this.$refs.innerSettleBillModal.sysUsers = this.sysUsers
      if (this.sysUsers.length > 0) {
        httpAction(this.url.getCustomerInfo + record.customerId,null,'GET').then((re) => {
          if(Number(re.code) === 200) {
            ...
            for(let i = 0; i < unitContracts.length; i++) {
              if(String(record.unitContractId) === String(unitContracts[i].id)) {
                record.unitContractId = unitContracts[i].contractName
              }
            }
            httpAction(this.url.getSettleDomainInfo + record.settleDomainId, null, 'GET').then((r) => {
              if (r.data.departSettlementInfo !== null && r.data.departSettlementInfo.taxFee !== null) {
                this.$refs.innerSettleBillModal.settleDomainInfo = r.data
                this.$refs.innerSettleBillModal.edit(record)
              } else {
    ...
    makeIncomeBelong(record) {
      this.$refs.incomeBelongModal.title = "收入归属设置";
      this.$refs.incomeBelongModal.sysUsers = this.sysUsers
      this.$refs.incomeBelongModal.organs = this.organs
      if (this.sysUsers.length > 0 && this.organs.length > 0) {
        httpAction(this.url.getCustomerInfo + record.customerId,null,'GET').then((re) => {
          if(Number(re.code) === 200) {
            const unitContracts = re.data.contractVoList || []
            for(let i = 0; i < unitContracts.length; i++) {
              if(record.unitContractId == unitContracts[i].id) {
                record.unitContractId = unitContracts[i].contractName
              }
            }
            httpAction(this.url.getSettleDomainInfo + record.settleDomainId, null, 'GET').then((r) => {
              if (r.data.departSettlementInfo !== null && r.data.departSettlementInfo.taxFee !== null) {
                this.$refs.incomeBelongModal.settleDomainInfo = r.data
                this.$refs.incomeBelongModal.edit(record)
              } else {
                this.$message.error('税率不能为空！')
              }
            });
