<template>
  ...
  >
                ...
                >
                  ...
                  <tr>
                    ...
                    <td
                      ...
                      style="padding-left: 10px;text-align:left;"
                    >{{ model.advanceMoney }}
                      <img src="/images/warning.png" @click="showList" v-if="model.existOverduePayment" width="22" style="margin-left:18px;cursor: pointer;vertical-align: middle">
                    </td>
    ...
    <overdue-list-modal ref="OverdueListForm"></overdue-list-modal>
...
export default {
  name: 'AdvancePaymentAuditModule',
  components: { AuditHeighest,PayBarChart,AuditNew,VueG6Editor,StatementDetailModal,OverdueListModal },
  data() {
    return {
      title: '操作',
      visible: false,
  ...
  methods: {
    ...
    showList(){
      this.$refs.OverdueListForm.show(this.model.settleDomainId)
    },
    handleTableChange(pagination){
      this.salaryIpagination = pagination
      if(this.model.billType === '2'){
        this.getNotaxList(this.model.settleDomainId,pagination.current)
      }else{
    ...
    callback(key){
      if(String(key) === '2' && this.totalList.length === 0){
    ...
    handleOk() {
      if(String(this.model.oldProcesFlag) === '2' || String(this.model.oldProcesFlag) === '3'){
        this.$refs.AuditNewForm.show(this.model)
      }else if(String(this.model.oldProcesFlag) === '1'){
        const url = `${this.url.getNextNodes}?advancePaymentId=${this.model.id}`
        httpAction(url, null, 'GET').then((res) => {
          if(Number(res.code) === 200){
    ...
    close() {
      this.$emit('close')
      this.visible = false
      this.staticList = this.totalList = []
      this.salaryDataSource = []
