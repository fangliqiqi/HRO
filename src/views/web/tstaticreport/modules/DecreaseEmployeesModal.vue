...
export default {
  ...
  methods:{
    ...
    exportDecreaseEmploee(){
      ...
      httpAction(`/hrBase/tempreducereport/exportList?${params}`,null,'get').then(res=>{
        if(res.code === 200){
          ...
          const excelDatas = [{
            ...
            tableDatas: res.data,
            sheetName: "当月减少人员明细统计"
          }]
          this.json2excel(excelDatas, `档案月报-减员${param.month}`, true, "xlsx",true)
        }else{
          this.$message.warn(res.msg)
          this.confirmLoading = false
        }
