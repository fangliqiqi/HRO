  ...
  <a-card :bordered="false">
    ...
    <div class="table-operator">
      ...
      <a-button
        type="primary"
        icon="plus"
        @click="handleAdd('forward')"
        v-has="'wxhr:tinvoice_savePreInvoicing'"
      >预开票申请</a-button>
      <a-button
        type="primary"
        icon="plus"
        @click="handleAdd('predict')"
      >预估开票</a-button>
    </div>
      ...
      >
        ...
        >
          ...
          <a-popconfirm
            v-if="record.auditFlag==2"
            title="确定作废吗?"
            @confirm="() => handleNullify(record.id)"
            v-has="'wxhr:tinvoice_del'"
          >
            <a>作废</a>
          </a-popconfirm>
  ...
  export default {
    ...
    data() {
      return {
        ...
        defineColumns: {
          ...
          settleDomainNo: {width: 120}, invoiceType: {width: 80, type: 'define', options: []},
          auditFlag: {width: 80, type: 'tag', options: []}, applyDate: {width: 140}
        },
        // excel表头数据
        exportFields: [],
        // 表头
    ...
    methods: {
      ...
      handleAdd: function(type) {
        ...
        if (String(type) === 'add') {
          this.$refs.invoiceApplyAddModal.title = '开票申请'
          this.$refs.invoiceApplyAddModal.areaTrees = Vue.ls.get('glob_area')
          this.$refs.invoiceApplyAddModal.unitBelongs = this.unitOptions
          this.$refs.invoiceApplyAddModal.invKindOptions = this.invKindOptions
          this.$refs.invoiceApplyAddModal.goodsNameOptions = this.goodsNameOptions
          this.$refs.invoiceApplyAddModal.taxClassificationOptions = this.taxClassificationOptions
          this.$refs.invoiceApplyAddModal.add(1)
        } else if (String(type) === 'forward') {
          this.$refs.invoiceForwardApplyAddModal.title = '预开票申请'
          this.$refs.invoiceForwardApplyAddModal.areaTrees = Vue.ls.get('glob_area')
          this.$refs.invoiceForwardApplyAddModal.unitBelongs = this.unitOptions
          this.$refs.invoiceForwardApplyAddModal.invKindOptions = this.invKindOptions
          this.$refs.invoiceForwardApplyAddModal.goodsNameOptions = this.goodsNameOptions
          this.$refs.invoiceForwardApplyAddModal.taxClassificationOptions = this.taxClassificationOptions
          this.$refs.invoiceForwardApplyAddModal.add(1)
        } else if (String(type) === 'predict') {
          this.$refs.invoiceApplyAddModal.title = '预估开票'
          this.$refs.invoiceApplyAddModal.areaTrees = Vue.ls.get('glob_area')
          this.$refs.invoiceApplyAddModal.unitBelongs = this.unitOptions
          this.$refs.invoiceApplyAddModal.invKindOptions = this.invKindOptions
          this.$refs.invoiceApplyAddModal.goodsNameOptions = this.goodsNameOptions
          this.$refs.invoiceApplyAddModal.taxClassificationOptions = this.taxClassificationOptions
          this.$refs.invoiceApplyAddModal.predictAdd()
        }
      },
      // 详情
      ...
      handleRelease: function(record) {
        if (String(record.invoiceType) === '0') {  // 结算开票
          httpAction(`${this.url.queryAllInfoById}?id=${record.id}`, '', 'GET').then((res) => {
            if (Number(res.code) === 200 && res.data) {
              this.$refs.invoiceApplyDetailModal.title = '开票申请提交'
              this.$refs.invoiceApplyDetailModal.addConfirmButton = false
              this.$refs.invoiceApplyDetailModal.unitBelongs = this.unitOptions
              this.$refs.invoiceApplyDetailModal.invKindOptions = this.invKindOptions
              this.$refs.invoiceApplyDetailModal.invoiceTypeOptions = this.invoiceTypeOptions
          ...
          httpAction(`${this.url.queryAllInfoById}?id=${record.id}`, '', 'GET').then((res) => {
            if (Number(res.code) === 200 && res.data) {
              this.$refs.invoiceForwardApplyDetailModal.title = '预开票申请提交'
              this.$refs.invoiceForwardApplyDetailModal.addConfirmButton = false
              this.$refs.invoiceForwardApplyDetailModal.unitBelongs = this.unitOptions
              this.$refs.invoiceForwardApplyDetailModal.invKindOptions = this.invKindOptions
              this.$refs.invoiceForwardApplyDetailModal.invoiceTypeOptions = this.invoiceTypeOptions
      ...
      // 作废
      handleNullify(id){
        httpAction(this.url.cancel + id, {}, 'POST').then((res) => {
          if(Number(res.code) === 200){
