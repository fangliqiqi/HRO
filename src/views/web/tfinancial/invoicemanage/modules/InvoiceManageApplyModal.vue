  ...
  methods: {
    ...
    handleUnitChange(value) {
      ...
      if (Number(this.pageType) === 3) {
        ...
        // 结算单列表会跟随结算主体变化
        this.url.list = `${this.url.list.split('?')[0]}?type=1&unitId=${value}`
    ...
    copyInvoiceClick() {
      ...
      this.$refs.selectProductCategoryModal.isEstimate = Number(this.pageType) === 3
      this.$refs.selectProductCategoryModal.show(2)
    },
    // 开票复制选择回调
    productSelectedModalOk(data) {
      if (Number(this.pageType) === 3) {
        // 结算单列表会跟随客户单位变化
        this.url.list = `${this.url.list.split('?')[0]}?type=1&unitId=${data.unitId}`
        this.loadStatementListData()
        // 根据客户单位获取购买方配置数量
        httpAction(`${this.url.getInvoiceByUnit}?unitId=${data.unitId}`, null, 'GET').then(res => {
          if (Number(res.code) === 200 && res.data) {
            ...
            this.resList = [{id:data.unitId, title: data.unitName }]
            this.$nextTick(() => {
              this.form.setFieldsValue(pick(data, 'unitId', 'invoiceUser', 'auditUserId'))
            })
            this.buyerSetting = { total: res.data.length }
            this.invoiceCallbackAssignData(data)
          } else {
            this.$nextTick(() => {
              this.form.setFieldsValue(pick(data, 'unitId'))
            })
            this.buyerSetting = {}
            this.$message.error('请先完善开票信息配置！')
          }
        ...
        const resultList = this.unitBelongs.filter(item => {
          return String(item.id) === String(data.unitId)
        })
        this.settleDomainBelongs = []
        ...
        httpAction(`${this.url.getInvoice}?settleId=${data.settleDomainId}&current=1&size=30`, {}, 'GET').then(res => {
          if (Number(res.code) === 200 && res.data.total) {
            ...
            this.resList = [{id:data.unitId, title: data.unitName }]
            this.$nextTick(() => {
              this.form.setFieldsValue(pick(data, 'unitId', 'settleDomainId', 'invoiceUser', 'auditUserId'))
            })
            this.buyerSetting = { total: res.data.total }
            this.invoiceCallbackAssignData(data)
          } else {
            this.$nextTick(() => {
              this.form.setFieldsValue(pick(data, 'unitId', 'settleDomainId'))
            })
            this.buyerSetting = {}
            this.$message.error('请先完善开票信息配置！')
          }
        ...
        if (Number(this.pageType) !== 2) {
          // 结算单列表会跟随结算主体变化
          this.url.list = `${this.url.list.split('?')[0]}?type=0&deptId=${data.settleDomainId}&unitId=${data.unitId}`
          this.loadStatementListData()
        }
      }
    },
    ...
    buyerSettingDataAssign(total, configData) {
      ...
      if(String(configData.invKind) !== '51'){
        this.$refs.proDetailTableModal.forEach(item => {
          item.invoiceKind = configData.invKind
        })
      }
