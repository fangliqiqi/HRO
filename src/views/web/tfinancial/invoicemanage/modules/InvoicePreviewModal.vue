<template>
  <div style="overflow: hidden;position: relative;">
    ...
    <div class="invoicePreview">
      <div class="thead"><span id="tabledesc1">安徽增值税{{ String(invoiceInfo.tinvoice.invKind) === '0'?'专用':'普通' }}发票</span></div>
      <div class="ttop">
        ...
        <div class="ttopInfo">
          <p>
            <label for="name">名称</label>：<input
              ...
              id="name"
              autocomplete="off"
              :value="invoiceInfo.tinvoice.headerName"
            />
          </p>
          <p>
            <label for="nsr">纳税人识别号</label>：<input
              ...
              id="nsr"
              autocomplete="off"
              :value="invoiceInfo.tinvoice.taxIdentificationNum"
            />
          </p>
          <p>
            <label for="addr">地址、电话</label>：<input
              ...
              id="addr"
              autocomplete="off"
              :value="(invoiceInfo.tinvoice.address||'')+` `+(invoiceInfo.tinvoice.tel||'')"
            />
          </p>
          <p>
            <label for="bank">开户行及账号</label>：<input
              ...
              id="bank"
              autocomplete="off"
              :value="(invoiceInfo.tinvoice.bankName||'') +` `+(invoiceInfo.tinvoice.bankNo||'')"
            />
          </p>
        </div>
      ...
      <div class="table">
        <table class="goodList" aria-describedby="tabledesc1">
          ...
          <template v-if="invoiceInfo.tinvoice.listed == 1">
            <tr
          ...
          <template v-else>
            <tr>
              ...
              <td></td>
              <td></td>
              <td>{{ Number(invoiceInfo.tinvoice.invoiceMoney).toFixed(2) }}</td>
              <td></td>
              <td>{{ Number(invoiceInfo.tinvoice.allTax).toFixed(2) }}</td>
            </tr>
          </template>
        </table>
      </div>
      <div class="count">
        ...
        >
          <span>¥ {{ Number(invoiceInfo.tinvoice.invoiceMoney).toFixed(2) }}</span>
          <span>&nbsp;</span>
          <span>¥ {{ Number(invoiceInfo.tinvoice.allTax).toFixed(2) }}</span>
        </div>
        <div class="label">价税合计（大写）</div>
        <div class="font">
          <span>&nbsp;&nbsp;{{ invoiceInfo.tinvoice.chmoney }}</span>
          <span class="right">(小写) <span>¥ {{ invoiceInfo.tinvoice.invoiceMoney }}</span></span>
        </div>
      </div>
      <div class="ttop">
        ...
        <div class="ttopInfo customer">
          <p>
            <label>名称</label>：<span>{{ invoiceInfo.tinvoice.companySeller }}</span>
          </p>
          <p>
            <label for="nsr1">纳税人识别号</label>：<span>{{ invoiceInfo.tinvoice.taxNbSeller }}</span>
          </p>
          <p>
            <label for="addr1">地址、电话</label>：<input
              ...
              id="addr1"
              autocomplete="off"
              :value="(invoiceInfo.tinvoice.addressSeller||'')+` `+(invoiceInfo.tinvoice.telSeller||'')"
            />
          </p>
          <p>
            <label for="bank1">开户行及账号</label>：<input
              ...
              id="bank1"
              autocomplete="off"
              :value="(invoiceInfo.tinvoice.bankSeller||'')+` `+(invoiceInfo.tinvoice.bankNoSeller||'')"
            />
          </p>
        </div>
        <div class="ttopPass">&nbsp;备注</div>
        <div class="ttopBlank">{{ invoiceInfo.tinvoice.taxDeduction>0 ? (`差额征税：${invoiceInfo.tinvoice.taxDeduction + (invoiceInfo.tinvoice.remark||'')}`) : (invoiceInfo.tinvoice.remark||'') }}</div>
      </div>
      <div class="tfoot">
        ...
        <input
          ...
          type="text"
          class="text"
          :value="invoiceInfo.tinvoice.cashier"
        ...
        <input
          :disabled="!isCanModify"
          type="text"
          :value="invoiceInfo.tinvoice.checker"
          class="text"
          @blur.prevent="checkerOnblur($event)"
        />
        <span class="blue">开票人：</span><span class="text">{{ invoiceInfo.tinvoice.invoiceUserName }}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
        <span class="blue">销售方：(章)</span>
      </div>
      <div class="footext">税收分类编码版本号：{{ invoiceInfo.tinvoice.codeVersion }}</div>
    </div>
    <div class="rtxt">
      第一联：记账联 销货方记账凭证
    </div>
...
export default {
  ...
  methods: {
    ...
    cashierOnblur(e) {
      ...
      const tinvoice = this.invoiceInfo.tinvoice
      const cashier = tinvoice.cashier || ''
      if (String(cashier) !== String(value)) {
        const idString = this.invoiceInfo.tinvoice.id || ''
        const url = `${this.url.update}?id=${idString}&cashier=${value}`
        this.changeSomeField(url, '收款人')
      }
    },
    // 复核人输入框失去焦点
    checkerOnblur(e) {
