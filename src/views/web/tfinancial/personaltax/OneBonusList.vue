  ...
  <a-card :bordered="false">
    ...
    >
      ...
      <a-button
        type="primary"
        style="margin-left:10px"
        v-has="'wxhr:tstatisticsannualbonus_doDeleteByDeclareMonth'"
      ...
      >
        ...
        >
          ...
          >
            <a
              href="#"
              v-have="'wxhr:tstatisticsannualbonus_del'"
            >删除</a>
          </a-popconfirm>
        </span>
      </a-table>
  ...
  export default {
    ...
    data () {
      return {
        ...
        url: {
          list: '/salary/tstatisticsannualbonus/page',
          delete: '/salary/tstatisticsannualbonus/',
          doSubmit:'/salary/tstatisticsannualbonus/doStatisticsAnnualBonus',
          exportUrl: '/salary/tstatisticsannualbonus/doExportStatisticsAnnualBonus',
          deleteBatch:'/salary/tstatisticsannualbonus/doDeleteByDeclareMonth'
        },
      }
    ...
    methods:{
      ...
      handleExport() {
        ...
        getAction(this.url.exportUrl,params).then((res) => {
          const excelDatas = [
            {
              tHeader: ["税务主体",'结算主体','申报月份','姓名','身份证号','年终奖','年终奖单独扣税'], // 表一的数据字段
              filterVal: ['invoiceTitle','settleDepartName','declareMonth','empName','empIdcard','annualBonus','tax'], // sheet表一头部
              tableDatas: res.data, // 表一的整体json数据
              sheetName: "申报人员"// 表一的sheet名字
            },
          ]
          this.json2excel(excelDatas, "全年一次性奖金", true, "xlsx")
