  ...
  <a-card :bordered="false">
    ...
    <div>
      ...
      >
        ...
        >
          <a @click="handleDetail(record)">认领详情</a>
          <a-divider
            type="vertical"
            v-has="'wxhr:treciverecord_add'"
          />
          <a
          ...
          <a-divider
            type="vertical"
            v-has="'wxhr:treciverecord_addAdvancePayment'"
          />
          <a
            @click="handlePayments(record)"
            v-has="'wxhr:treciverecord_addAdvancePayment'"
          >垫付还款</a>
          <a-divider
            type="vertical"
            v-has="'wxhr:treciverecord_insurance'"
          />
          <a
    ...
    <!-- 表单区域 -->
    <wait-receive-add-modal
      ref="waitReceiveAddModal"
      @ok="modalFormOk"
    ></wait-receive-add-modal>
    <wait-receive-payments-modal
      ref="waitReceivePaymentsModal"
      @ok="modalFormOk"
    ></wait-receive-payments-modal>
...
<script>
  ...
  import WaitReceiveAddModal from './modules/WaitReceiveAddModal'
  // import { getSettleDomainSelectVos } from '@/api/api'
  import WaitReceivePaymentsModal from './modules/WaitReceivePaymentsModal'
  ...
  export default {
    name: 'WaitReceiveList',
    mixins: [JeecgListMixin],
    components: {
      WaitReceiveAddModal,
      WaitReceivePaymentsModal,
    ...
    data() {
      return {
        ...
        reciveTypeOptions: {'1': '薪资及其他', '2': '纯收入', '3': '事务性外包','5':'财务账单'}, // 款项类型
        statusOptions: {
          '0': {'option': '发布认领', 'color': 'cyan'},
          '1': {'option': '部分认领', 'color': 'purple'},
          '2': {'option': '全部认领', 'color': 'green'},
          '3': {'option': '入账退回', 'color': 'green'}
        },
        // excel表头数据
        exportFields: [
          '来款单位', '进账金额', '进账时间', '发布人', '发布时间', '财务说明', '待认领金额', '已认领金额','到款单位',
        ],
        // 表头
        columns: [
          {
        ...
        url: {
          list: 'salary/trecive/page',
          delete: 'salary/trecive/',
          exportXlsUrl: 'salary/trecive/doexport?',
    ...
    methods: {
      ...
      handleAdd: function(record) {
        this.$refs.waitReceiveAddModal.title = '款项认领'
        this.$refs.waitReceiveAddModal.reciveTypeOptions = this.reciveTypeOptions
        this.$refs.waitReceiveAddModal.settleDomainBelongs = this.settleDomainOptions
        if (Object.keys(this.settleDomainOptions).length > 0) {
          httpAction(this.url.queryAllInfoById + record.id, '', 'GET').then((res) => {
            if (Number(res.code) === 200 && res.data) {
              this.$refs.waitReceiveAddModal.edit(res.data)
            } else {
              this.$message.error('数据请求失败！')
            }
          })
      ...
      // 垫付还款
      handlePayments: function(record) {
        this.$refs.waitReceivePaymentsModal.title = '垫付还款'
        httpAction(this.url.queryAllInfoById + record.id, '', 'GET').then((res) => {
          if (Number(res.code) === 200 && res.data) {
            this.$refs.waitReceivePaymentsModal.edit(res.data)
          } else {
            this.$message.error('数据请求失败！')
          }
        })
      ...
      handleInsurance: function(record) {
        this.$refs.waitReceivePaymentsModal.title = '商险预估还款'
        this.$refs.waitReceivePaymentsModal.paymentsStatus = 2  // 商险预估还款
        httpAction(this.url.queryAllInfoById + record.id, '', 'GET').then((res) => {
          if (Number(res.code) === 200 && res.data) {
            this.$refs.waitReceivePaymentsModal.edit(res.data)
          } else {
      ...
      handleExport: function() {
        ...
        this.$refs.receiveRecordExportModal.title = '待认领款项导出'
