<template>
  <a-card :bordered="false">
      ...
      >
        ...
        >
          <a
            ...
            v-has="'wxhr:tProvidentaprovalform_outAccount'"
          >出账</a>
        </span>
      </a-table>
    </div>
    ...
    <!-- 表单区域 -->
    <fund-approval-salary-detail-modal
      ref="fundApprovalSalaryDetailModal"
      @ok="modalFormOk"
    ></fund-approval-salary-detail-modal>
  </a-card>
</template>
...
<script>
  ...
  import FundApprovalSalaryDetailModal from './modules/FundApprovalSalaryDetailModal'

  export default {
    name: 'FundApprovalSalaryList',
    mixins: [JeecgListMixin],
    components: {
      FundApprovalSalaryDetailModal,
    },
    data() {
      return {
        description: '公积金核准表出账',
        // 枚举字段
        areaTrees: [],  // 地区
        areaCitys: [],  // 城市
        areaTowns: [],  // 区县
        ...
        url: {
          list: 'hrSocial/tprovidentaprovalform/page?status=1',
    ...
    methods: {
      ...
      handleSalaryOut: function(record) {
        this.$refs.fundApprovalSalaryDetailModal.title = '公积金核准表详情'
        this.$refs.fundApprovalSalaryDetailModal.payTypeBelongs = this.payTypeOptions
        if (this.payTypeOptions.length > 0) {
          httpAction(`${this.url.queryAllInfoById}?id=${record.id}`, '', 'GET').then((res) => {
            if (Number(res.code) === 200 && res.data) {
              this.$refs.fundApprovalSalaryDetailModal.edit(res.data)
            } else {
