  ...
  export default {
    ...
    methods: {
     ...
     async edit(record) {
        ...
        this.visible = true
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model, 'empName', 'empIdcard', 'type', 'bankName', 'bankNo', 'money', 'reason'))
        })
      ...
      removeFile(file){
        if(file.id){
          httpAction(`/salary/lostandfoundinfo/ossFileDelete/${file.uid}`,{},'GET').then(res=>{
            if(res.code === 200){
