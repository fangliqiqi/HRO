...
export default {
  ...
  methods:{
    ...
    async show(record){
      ...
      httpAction(this.url.openBilling + '?lostAndFoundInfoId=' + record.id, {}, 'POST').then((res) => {
        if(res.code == 200){
          ...
          this.copyText = this.lostInfo.empName + " " + this.lostInfo.money + " " +  this.lostInfo.bankNo
    ...
    handleDownload(){
      ...
      var excelDatas = [
        {
          tHeader: ["员工姓名", "身份证号", "开户行","银行账号", "发放金额"],
          filterVal: ['empName','empIdcard', "bankName", "bankNo","money"],
          tableDatas: [{
            'empName':this.lostInfo.empName,'empIdcard':this.lostInfo.empIdcard,'bankName':this.lostInfo.bankName,
    ...
    changeTab(key){
      ...
      if(key == 2){
        ...
        this.$refs.salaryFailRecordModal.show()
      }
    },
    hancleCopy(){
