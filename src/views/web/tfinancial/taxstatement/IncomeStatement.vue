  ...
  export default {
    ...
    data() {
      return {
        ...
        btnLoading:false,
        url: {
          list: 'salary/tstatisticsemployeesum/page?orders%5B0%5D.asc=false&orders%5B0%5D.column=settle_month&orders%5B1%5D.asc=false&orders%5B1%5D.column=id',
          exportUrl:'salary/tstatisticsemployeesum/getTStatisticsEmployeeSumList',
        },
        ...
        httpAction(httpurl,{},'get',60000).then(res=>{
          if(res.code === 200){
            const arr = res.data.map(item=>{
              ...
              item.taxFee = item.taxFee == 2 ? '-' : `${item.taxFee}%`;
              return item;
            })
            const excelDatas = [{
              tHeader: ['结算周期', '服务对象','客户编码','服务收入','薪资类型','发薪人数','个税申报总额',
              '工资总额','参保人数','扣缴社保总额(个人部分)','单位缴纳社保金额','税率'],
              filterVal: ['settleMonth','unitName','unitNo', 'settleMoney','salaryTypeName','salaryNums','taxMoney',
              'realaryMoney','socialNums','personSocial','unitSocial','taxFee'],
              tableDatas: arr,
              sheetName: '员工收入汇总统计表'
            }]
            _this.json2excel(excelDatas, `员工收入汇总统计表`, true, "xlsx");
          }else{
