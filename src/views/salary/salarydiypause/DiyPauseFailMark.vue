<template>
  <a-card :bordered="false">
            ...
            >
              <a-form-item label="数据来源" class="labTxt">
                <a-select
                  ...
                  placeholder="请选择"
                >
                  <template v-for="(item, index) in sourceOption">
                    <a-select-option
                      :key="index"
                      :value="index"
                      v-if="item"
      ...
      >
        ...
        >
          <a
            ...
            v-if="hasOperatePermission('wxhr:lostandfoundinfo_add')"
          >发放失败</a>
        </span>
    ...
    <pause-grant-faild-modal :title="title" ref="GrantFaildForm" @ok="modalFormOk"></pause-grant-faild-modal>
...
import { hasOperatePermission,limitAccount } from '@/utils/common'
import SelectPage from '@/components/jeecg/SelectPage'
import PauseGrantFaildModal from './modules/PauseGrantFaildModal'

export default {
  name: 'DiyPauseFailMark',
  mixins: [JeecgListMixin],
  components:{
    PauseGrantFaildModal,
    SelectPage
  },
  data(){
    return{
      ...
      columns:[
        ...
        //   ellipsis: true,
        //   customRender: text => {
        //     const resultText = this.sourceOption[text]
      ...
      url:{
        list: '/salary/tchargerecord/getPauseDiyTagPage?orders%5B0%5D.asc=true&orders%5B0%5D.column=revenue_date',
