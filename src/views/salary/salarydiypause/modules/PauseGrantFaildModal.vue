<template>
  ...
  >
    <a-form layout="inline" :form="form">
      <a-row :gutter="24">
        <a-col :span="8">
          <a-form-item label="自定义项名称" class="labelTxt bankName">{{ model.name || '-' }}</a-form-item>
        </a-col>
        <a-col :span="8">
          <a-form-item label="收款单位" class="labelTxt bankName">{{ model.orgName || '-' }}</a-form-item>
        </a-col>
        <a-col :span="8">
          <a-form-item label="发放方式" class="labelTxt bankName">{{ changeTypeOption[model.chargeType] || '-' }}</a-form-item>
        </a-col>
      </a-row>
      <a-row :gutter="24">
        <a-col :span="8">
        ...
        <a-col :span="8">
          <a-form-item label="银行账号" class="labelTxt bankName">{{ model.bankNo || '-' }}</a-form-item>
        </a-col>
        <a-col :span="8">
          <a-form-item label="发放金额" class="labelTxt">{{ model.chargeMoney == null ? '-' : model.chargeMoney }}</a-form-item>
        </a-col>
      </a-row>
      <a-row :gutter="24">
        <a-col :span="24">
...
export default {
  ...
  methods:{
    ...
    beforeUpload(file){
      ...
      this.fileList = [...this.fileList, {uid:file.uid,status:'done',name:file.name,url:URL.createObjectURL(file),response:'{"status": "success"}'}];
      return false;
    },
    removeFile(file){
