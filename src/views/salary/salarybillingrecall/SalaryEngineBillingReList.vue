    ...
    <div>
      ...
      >
        ...
        >
          <a
            type="primary"
            v-has="'wxhr:tsalaryengineering_goBack'"
...
<script>
  ...
  import SalaryEngineAuditModal from '../salaryengineaudit/modules/SalaryEngineAuditModal'
  import SelectPage from '@/components/jeecg/SelectPage'

  export default {
    name: "SalaryEngineBillingRecallList",
    ...
    data () {
      return {
        ...
        url: {
          list: '/salary/tsalaryengineering/gobackSalaryStandardPage',
    ...
    methods: {
      ...
      changeMoney(e){
        ...
        if(matchArr){
          ...
          if(matchArr.length == 1){
            const str = settlementAmount.split(".")[1]
            const reg2 = /^[0-9]*$/
            if(!reg2.test(str)){
              this.queryParam.settlementAmount = ''
            }else{
      ...
      handleBackOk() {
        ...
        this.backform.validateFields((err,values) => {
          if(!err){
            ...
            httpAction(url,{'id':this.salaryStandardInfo.id,'status':3,'remark':values.remark},'POST').then((res) => {
              if(Number(res.code) === 200){
