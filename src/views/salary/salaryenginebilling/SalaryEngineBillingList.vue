<template>
  <a-card :bordered="false">
      ...
      >
        ...
        >
          <a
            ...
            v-has="'wxhr:tsalaryengineerin_doBilling'"
          >出账</a>
          <a
    ...
    <!-- 表单区域 -->
    <salary-engine-billing-modal
      ref="SalaryEngineBillingModal"
...
<script>
  ...
  import SalaryEngineBillingModal from './modules/SalaryEngineBillingModal'
  ...
  export default {
    ...
    data () {
      return {
        ...
        url: {
          list: '/salary/tsalaryengineering/billingSalaryEngineeringPage?isBacked=0',
          ...
          openBill:'/salary/engineer/doExportSalary', // 详情
          backUrl:'/salary/tsalaryengineering/doBilling',
        }
      }
    },
    created () {
    ...
    methods: {
      getSettleList(){
        getAction(this.url.settleList).then((res) => {
          this.settleList = res
        })
      },
      getOrgForCusAll(){
      ...
      handleSumbit(record) {
        ...
        httpAction(url,{},'POST').then((res) => {
          if(Number(res.code) === 200){
            this.$refs.SalaryEngineBillingModal.title = '发放出账'
            this.$refs.SalaryEngineBillingModal.settlementList = this.settlementList
            this.$refs.SalaryEngineBillingModal.employData = this.employData
            this.$refs.SalaryEngineBillingModal.show(record,res.data || {})
          }else {
      ...
      handleBack(record){
        ...
        httpAction(url,{},'POST').then((res) => {
          if(Number(res.code) === 200){
            this.$refs.backModal.show(record,res.data.engineerSettlementForm.id)
          }else {
            this.$message.error(res.msg || '详情获取失败！')
          }
        })
