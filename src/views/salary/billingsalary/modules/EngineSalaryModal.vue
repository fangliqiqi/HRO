...
import EngineSettleAccountModal from './EngineSettleAccountModal'
...
export default {
  name: 'EngineSalaryModel',
  components: {
    EngineSettleAccountModal,
    BillingPageModal,
    DetailListItem,
    DetailList,
  ...
  data() {
    return {
      ...
      url:{
        ...
        mathDetailUrl:'/salary/treciverecord/getMatchingByMatchId'
      },
  ...
  methods: {
    ...
    show(record,detailInfo) {
      ...
      this.settlementhFormInfo = detailInfo.engineerSettlementForm // 结算单数据
      this.salaryAccountOne =detailInfo.engineerAccountList?detailInfo.engineerAccountList[0]:{} 
      this.dataSourceList = this.detailData.engineerAccountList // 工资明细列表数据
    ...
    handleDownload() {
      ...
      httpAction(url,{},'POST').then((res) => {
        if(Number(res.code) === 200){
          ...
          let createName = ''
          let revenueName = ''
          // console.log(res.data.salary.createUser);
