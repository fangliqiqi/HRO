...
/**
 * 过滤账户是否拥有某一个权限 - 为页面判定权限服务
 *
 * @param permission
 * @param route
 * @returns {boolean}
 */
export function hasPermissionToPage(permission, route) {
  
  if (route.meta && route.meta.permissionList) {
    for (let i = 0, len = route.meta.permissionList.length; i < len; i++) {
      if(route.meta.permissionList[i].action == permission) {
        return true;
      }
    }
    return false
