...
<script>
  ...
  export default {
    ...
    created() {
      ...
      for(let j = 0,len=this.permissionMenuList.length; j < len; j++) {
        if(this.permissionMenuList[j].name == keyObj.key1){
          keyObj.num++
        }
        if(this.permissionMenuList[j].name == keyObj.key2){
          keyObj.num++
          keyObj.index = j
        }
      }
      if(keyObj.num == 2){
        this.$delete(this.permissionMenuList,keyObj.index)
      }
    ...
    methods: {
      ...
      toggle() {
        console.log(this.layoutMode,this.device,234)
