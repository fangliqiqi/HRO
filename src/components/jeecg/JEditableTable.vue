...
<template>
  <a-spin :spinning="loading">
    ...
    <div v-if="actionButton" class="action-button">
      <a-button type="primary" icon="plus" v-if="rows.length < maxRows" @click="handleClickAdd">新增</a-button>
      <span class="gap"/>
      <template v-if="selectedRowIds.length>0 && isAllowDel">
        <a-popconfirm
                  ...
                  <a-tooltip
                    ...
                    <span
                      ...
                      <j-month
                        ...
                        :placeholder="replaceProps(col, col.placeholder)"
                        :trigger-change="true"
                        :dateFormat="col.type === formTypes.month? 'YYYYMM':'YYYY-MM'"
                        @change="(v)=>handleChangeJDateCommon(v,id,row,col)"/>
  ...
  export default {
      ...
      dataSource: {
        handler: function(newValue) {
          ...
          newValue.forEach(data => {
            let value = { id: _this.caseId + data.id }
            let row = { id: value.id }
            let disabled = false
            _this.columns.forEach(column => {
              ...
              } else if (column.type === FormTypes.date || column.type === FormTypes.datetime || column.type === FormTypes.month) {
              ...
              // 解析disabledRows
              for (let columnKey in _this.disabledRows) {
                // 判断是否有该属性
                if (_this.disabledRows.hasOwnProperty(columnKey) && data.hasOwnProperty(columnKey)) {
                  // row[columnKey] =
                  if (disabled !== true) {
                    disabled = _this.disabledRows[columnKey] === data[columnKey]
                    if (disabled) {
    ...
    methods: {
      ...
      add(num = 1, forceScrollToBottom = false) {
        ...
        let rows = this.rows
        for (let i = 0; i < num; i++) {
          let row = { id: `${this.caseId}${timestamp}${rows.length}_add` }
      ...
      removeSelectedRows() {
        ...
        for(let i = 0; i < this.selectedRowIds.length; i++) {
          idArray.push(this.selectedRowIds[i].split('-')[2]);
        }
        
        // if(this.dataSource != null && this.dataSource.length > 0) {
        //   // eslint-disable-next-line no-unused-vars
        //   this.dataSource.find(function(value, index, arr) {
        //     console.log(idArray)
        ...
        if(ids.indexOf('ids') >= 0 && this.url.batchDelete) {
          httpAction(this.url.batchDelete + "?" + ids,null, 'DELETE').then((res) => {
            if(res.code == 200){
      ...
      removeRows(id) {
        ...
        ids.forEach(removeId => {
          ...
          // 删除type为select
          const deleteSelect = (arr) => {
            let keys = Object.keys(arr)
            for (let i = 0; i < keys.length; i++) {
              if (keys[i] == 'type'+removeId) {
                delete(arr[keys[i]])
                return true
              }
            }
          ...
          deleteSelect(this.selectValues)
      ...
      getValuesSync(options = {}) {
        ...
        for (let value of inputValues) {
          ...
          // if(value.id.indexOf('_jet') >= 0) {
          //   delete value.id;
          // }
          // for(let i = 0; i < this.dataSource.length; i++) {
          //   if(value.coverName == this.dataSource[i].coverName && value.type == this.dataSource[i].type) {
          //     value.id = this.dataSource[i].id;
          //   }
          // }

          //匹配出数据id
          if(value.id.indexOf('_jet') >= 0) {
            let id = value.id.match(/\d{15,}/)
            if(id){
              value.id = id[0]
            }else{
              delete value.id;
      ...
      validateValue(rules, value) {
        ...
        for (let rule of rules) {
          ...
          if (rule.pattern && !isNull) {
            ...
            message = rule.message
            break
          } else if (rule.unique === true){
            // console.log(this.selectValues,111111)
            // type为select时验证唯一性
            let arr = Object.values(this.selectValues)
            passed = !this.isRepeatArray(arr)
            message = rule.message
          }
        }
      ...
      updateFormValues() {
        ...
        newTrEls.forEach(tr => {
          let { idx } = tr.dataset
          // console.log(idx,234)
          let value = _this.inputValues[idx]
          for (let key in value) {
            if (value.hasOwnProperty(key)) {
              ...
              if (el) {
                el.value = value[key]
                // if(el.nodeName == 'SPAN' && key != 'action'){
      ...
      handleConfirmDelete() {
        if ((this.getValuesSync().values.length - this.selectedRowIds.length) < this.deleteLength) {
          this.$message.error('请保留' + this.deleteLength + '行数据！')
          return false
        }
