<template>
  <a-row :gutter="gutter">
    ...
    <a-col :span="24" class="result">
      总共查询到<strong>{{ total }}</strong>条 
      <div>
        <a-button type="link" size="small" @click="pre()">上一个</a-button>
        <a-button type="link" size="small" @click="next()">下一个</a-button>
      </div>
    </a-col>
    <a-col
      :span="24"
...
export default {
  ...
  methods: {
    ...
    searchNode(value){
      ...
      this.updateNodes(false,this.selectList,1,'')
      
      if(val){
        this.ztreeObj.expandAll(false)
        this.selectList = this.ztreeObj.getNodesByParamFuzzy('title', val)
        
        if(this.selectList.length){
          for( var i=0, l=_this.selectList.length; i<l; i++) {
            const currentNode = _this.selectList[i];
            // 判断当前节点是否是顶级节点，如果不是查找顶级节点
            if(!currentNode.isParent){
              const parentNode = _this.ztreeObj.getNodeByTId(currentNode.parentTId);
              if(parentNode){
                if(parentNode.isParent){
                  _this.ztreeObj.expandNode(parentNode, true, true, true)
                }else{
                  const gradparentNode = _this.ztreeObj.getNodeByTId(parentNode.parentTId);
                  _this.ztreeObj.expandNode(gradparentNode, true, true, true)
                }
              }
              
            }else{
              _this.ztreeObj.expandNode(currentNode, true, true, true)
            }
            
          }
        }
        this.total = this.selectList.length
        this.updateNodes(true,this.selectList,1,'')
      }else{
        return false
      }
      
    },
    updateNodes(highlight,selectList,type,index){
      
      if(type == 1) {
        if(highlight){
          this.ztreeObj.setting.view.fontCss = {color:"#ef2929","font-weight":"normal"}
        }else{
          this.ztreeObj.setting.view.fontCss = {color:"#333","font-weight":"normal"}
        }
        for( var i=0, l=selectList.length; i<l; i++) {
          selectList[i].highlight = highlight
          this.ztreeObj.updateNode(selectList[i])
        }
      }else{
        this.ztreeObj.setting.view.fontCss = {color:"#ef2929","font-weight":"normal"}
        selectList.forEach(item => {
          this.ztreeObj.updateNode(item)
        });
        // 当前节点
        this.ztreeObj.setting.view.fontCss = {color:"#ef2929","font-weight":"bold"}
        this.ztreeObj.updateNode(selectList[index])
      }
    },
    getFontCss(treeId,treeNode){
      // eslint-disable-next-line
			return (!!treeNode.highlight) ? {color:"#ef2929"} : {color:"#333"};
    },
    pre(){
      let index = this.current % this.selectList.length
      this.ztreeObj.expandNode(this.selectList[index], true, true, true)
      this.updateNodes(true,this.selectList,3,index)
      if(this.current == 0){
        this.current = this.selectList.length-1
      }else{
        this.current--
      }
    },
    next(){
      let index = this.current % this.selectList.length
      this.ztreeObj.expandNode(this.selectList[index], true, true, true)
      this.updateNodes(true,this.selectList,2,index)
      if(this.current == this.selectList.length-1){
        this.current = 0
      }else{
        this.current++
      }
