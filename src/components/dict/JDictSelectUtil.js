...
export function filterDictText(dictOptions, text) {
  ...
  dictOptions.forEach(function (option) {
    if (String(text) === String(option.value)) {
      re = option.label;
    }
  });
  return re;
 ...
 * @param {*} text 
 */
export function filterDictCom(dictOptions, text,key) {
  let re = {};
  if (dictOptions.length > 0) {
    for (let i = 0; i < dictOptions.length; i++) {
      if (text == dictOptions[i][key]) {
        re = dictOptions[i];
      }
    }
  }
 ...
 * @return String
 */
export function organDict(dictOptions, text) {
  let re = "-";
  if (dictOptions.length > 0) {
    for (let i = 0; i < dictOptions.length; i++) {
      if (text == dictOptions[i].id) {
...
/**
 * 根据id获取客户单位名称
 * @param 字典树组 dictOptions 
 * @param 字典值 text 
 */
let customer = ''
export function customerDict(dictOptions, text) {
  ...
  for(let item of dictOptions){
    ...
    if(item.children.length){
      customerDict(item.children,text)
    }
  }
  return customer
}
