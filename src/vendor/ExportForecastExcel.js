...
function sheet_from_array_of_arrays(data, opts,autoLength) {
  ...
  let style0 = { border: border, alignment: { horizontal: 'center', wrapText: true, vertical: "center" }, font: { sz: 11, color: { rgb: "000000" }, outline: true }, fill: { bgColor: { indexed: 64 }, fgColor: { rgb: "ffffff" } } };
  ...
  if (opts == 0) {
    for (var i = 0; i < 54; i++) {
      ...
      if(i<26){
        ...
        order = String.fromCharCode(66,(65 + i-52))
      }else{
        order = String.fromCharCode(65,(65 + i-26))
      }
      let header = order + 1
      ws[header].s = style9
  ...
  //商险明细
  if (opts == 1) {
    for (let m1 = 0; m1 < 16; m1++) {
      let order1 = String.fromCharCode((65 + m1))
      for (let i1 = 1; i1 <= data.length; i1++) {
        ws[order1 + i1].s = style6
      }
    }
  }
  if (opts == 2) {
    for (let m2 = 0; m2 < 15; m2++) {
      let order2 = String.fromCharCode((65 + m2))
      for (let i2 = 1; i2 <= data.length; i2++) {
        ws[order2 + i2].s = style6
      }
    }
  }
  // 结算单数据整理
  if (opts == 3) {
    ...
    // 自定义项
    const customLength = autoLength.customItemLength
    ...
    const invoiceLeng = autoLength.invoiceInfo
    ...
    // 内容
    nextIndex = nextIndex +1
    const reciveLength = autoLength.reciveList
    ...
    for (var m = 0; m < 6; m++) {
      ...
      // ws[order + (less + 17 + 10)].s = style0
      // ws[order + (less + 17 + 11)].s = style0
      ws["A1"].s = style3
    ...
    for (let i = 1; i < colWidth.length; i++) {
      for (let j = 0; j < colWidth[i].length; j++) {
        if (result[j]['wch'] < colWidth[i][j]['wch']) {
          result[j]['wch'] = colWidth[i][j]['wch']+10;
        }
      }
    }
    ws['!cols'] = result;
...
export function export_json_to_excel_more_sheet({
  ...
  for (var j = 0; j < header.length; j++) {
    ...
    ws.push(sheet_from_array_of_arrays(data[j], j,length))
  }
  if (merges.length > 0) {
    if (!ws["!merges"]) ws["!merges"] = []
    merges.forEach(item => {
  ...
  if (autoWidth) {
    /*设置worksheet每列的最大宽度*/
    var colWidth = []
    for (var k = 0; k < header.length; k++) { // 50
      colWidth.push(
        data[k].map(row =>
          row.map(val => {
            /*先判断是否为null/undefined*/
