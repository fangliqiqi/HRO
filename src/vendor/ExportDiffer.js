...
function sheet_from_array_of_arrays(data, opts,refetLen) {
  // console.log(data)
  // console.log(opts);
  // console.log(refetLen);
  var ws = {};
  var range = {
    s: {
      c: 10000000,
  ...
  if (opts == 0) {
    ...
    // console.log(refetLen);
    let ref = Object.values(refetLen)
    // 合并项 arr 计算出合并开始项
    let arr = []
    ref.forEach((item,index)=>{
      if(index==0){
        ...
        arr.push(ref[index-1]+arr[index-1])
      }
      for(let s=0;s<15;s++){
        ws['!merges'].push({ s: { r:arr[index] , c: s }, e: { r: arr[index]+ref[index]-1, c: s } }) 
      }
    })
  ...
  // 结算单数据整理
  if (opts == 1) {
    let openNum = data[data.length - 1][0]
    ws['!merges'] = [
      { s: { r: 0, c: 0 }, e: { r: 0, c: 4 } },
      { s: { r: 1, c: 1 }, e: { r: 1, c: 2 } },
      { s: { r: 2, c: 1 }, e: { r: 2, c: 2 } },
    ...
    let less = data.length - 13 - 9 - openNum
    if (less > 0) {
      for (var l = 1; l <= less; l++) {
        ws['!merges'].push({ s: { r: l + 12, c: 1 }, e: { r: l + 12, c: 2 } })
      }
    }
    ws['!merges'].push({ s: { r: less + 12 + 1, c: 1 }, e: { r: less + 12 + 1, c: 2 } })
    ws['!merges'].push({ s: { r: less + 12 + 2, c: 1 }, e: { r: less + 12 + 2, c: 2 } })
    ws['!merges'].push({ s: { r: less + 12 + 3, c: 1 }, e: { r: less + 12 + 3, c: 2 } })
    ws['!merges'].push({ s: { r: less + 12 + 4, c: 1 }, e: { r: less + 12 + 4, c: 2 } })
    ws['!merges'].push({ s: { r: less + 12 + 5, c: 1 }, e: { r: less + 12 + 5, c: 2 } })
    ws['!merges'].push({ s: { r: less + 12 + 6, c: 1 }, e: { r: less + 12 + 6, c: 2 } })
    ws['!merges'].push({ s: { r: less + 12 + 7, c: 1 }, e: { r: less + 12 + 7, c: 2 } })
    ws['!merges'].push({ s: { r: less + 12 + 8, c: 1 }, e: { r: less + 12 + 8, c: 2 } })
    ws['!merges'].push({ s: { r: less + 12 + 9, c: 2 }, e: { r: less + 12 + 9, c: 4 } })
    // ws['!merges'].push({ s: { r: less + 17 + 9, c: 1 }, e: { r: less + 17 + 9, c: 5 } })
    // ws['!merges'].push({ s: { r: less + 17 + 10, c: 1 }, e: { r: less + 17 + 10, c: 3 } })

    // ws['!merges'].push({ s: { r: less + 17 + 11, c: 4 }, e: { r: less + 17 + 11, c: 5 } })
    // let less2 = data[data.length - 1][1]
    // if (less2 > 0) {
    //   for (let i = 1; i <= less2; i++) {
    //     ws['!merges'].push({ s: { r: less + 17 + 11 + i, c: 4 }, e: { r: less + 17 + 11 + i, c: 5 } })
    //   }
    // }
    ws['!merges'].push({ s: { r: less + 12 + 12, c: 3 }, e: { r: less + 12 + 12, c: 4 } })
    for (let j = 1; j <= (openNum - 4); j++) {
      ws['!merges'].push({ s: { r: less + 12 + 12 + j, c: 3 }, e: { r: less + 12 + 12 + j, c: 4 } })
    }
    for (var m = 0; m < 5; m++) {
      ...
      if (order == 'A' || order == 'E') {
        ...
        if (ws[order + '2'] != undefined) { ws[order + '2'].s = style4 }
        if (ws[order + '3'] != undefined) { ws[order + '3'].s = style4 }
        if (ws[order + '4'] != undefined) { ws[order + '4'].s = style4 }
      }
      ws[order + '5'].s = style5
      ...
      ws[order + (less + 12 + 11)].s = style0
      ws[order + (less + 12 + 12)].s = style0
      ws["A1"].s = style3
      ws[order + (data.length)].s = { border: border, font: { sz: 9, color: { rgb: "FFFFFF" }, outline: true } };
    }
    // for (let i = 18; i <= (8 + less + 18); i++) {
    //   ws["B" + i].s = style17
    // }
...
export function export_json_to_excel_more_sheet({
  ...
  for (var j = 0; j < header.length; j++) {
    ...
    if (j > 5 + coverLength && j <= header.length - insLength - 1) {
      ...
      ws.push(sheet_from_array_of_arrays(data[j], j,refetLen))
    }
  }
  if (merges.length > 0) {
    if (!ws["!merges"]) ws["!merges"] = []
